...
//echo '<div style="background-color:#fff">'.$pms_config['url_request']."GetRoomGuestInfo?RoomNo=".$room_name.'</div>';exit;
		if(curl_errno($ch))
		{
			//print curl_error($ch);
			return false;
	...
	else // jika koneksi ke PMS berhasil
	{
		...
		if($amount < 0) {
			$this->bill_data['credit'] += (float) $amount;
		} else if($amount > 0) {
			$this->bill_data['debit'] += (float) $amount;
		}
