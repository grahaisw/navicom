...
class pmsal_fids extends pmsal
{
	...
	if($total_airport > 0) {
		...
		if($total_flight > 0) {
			...
			$error = $error + 1;
			$message = 'Flight number is not valid!';
		}
	} else {
		$error = $error + 1;
	...
	$dates = new DateTime($new_time, new DateTimeZone('Asia/Makassar'));
	    ...
	    $testdate = strtotime('+16 hour',$fids_time2);
	    $mindate = strtotime('-1 hour',$fids_time2);
	    /** end **/	
            $jy = date('y',$fids_timestamp);
            $jd = date('z',$fids_timestamp)+1;
            $jd = str_pad($jd, 3, "0", STR_PAD_LEFT);
		...
		if($total_flight > 0) {
			...
			while($row = $db->sql_fetchrow($result)) {
				...
				$flight[] = array (
					...
					'Type'				=> (string) $type,
					'Timestamp'				=> (integer) $row['fids_timestamp'],
				);
			}
	...
	$fidscount = mktime($dates->format('H')-1, $dates->format('i'), $dates->format('s'), $dates->format('m'), $dates->format('d'), $dates->format('Y'));	
	...
	if(!empty($airport_code)) {
		...
		if(!empty($fids_id)) {
			...
			$sql_update = "UPDATE " . AIRPORT_FIDS_TABLE . " SET fids_lastupdate = ".time().", fids_changed = ".$fids_changed.", fids_lastupdate_microtime = ".$micro_time." WHERE fids_id = ".$fids_id;
