...
//echo '<font color="white">'.$this->bill_data[$i]['debit'].' '.$this->bill_data[$i]['credit'].'<font>'; exit;
	...
	while($row = $db->sql_fetchrow($result))
	{
		...
		{ 
			...
			$this->room_data[$i]['full_name'] = (string) $xml->Entry->GuestName;
			...
			if(trim($row->ItemName) != "") {	
				if($menu_code != "") { //echo 'a';exit;
					if(trim($row->ItemCode) == $menu_code) {
						...
						$this->menu_data[0]['tax_info'] = (string) trim($row->TaxInfo);
					...
					$this->menu_data[$i]['tax_info'] = (string) trim($row->TaxInfo);
	...
	{ 
		$sqlUpdate = "UPDATE ".SERVICES_TABLE." SET service_enabled = 0 WHERE service_enabled = 1";		
