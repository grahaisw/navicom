...
$flight_strange = array("FS410", "QG307", "ID6284" );
...
//echo "\n\n$i\n";		
	...
	$file = fopen("grab_ais.txt","a+");
	$time_start = "______Start at: ".date("Y-m-d H:i:s")."______\n";
	fwrite($file,$time_start);
...
//	echo 'name: ' . $string['Flight']['Contents']['Content'][0]['name']. '<br>';exit;
	...
	foreach($string as $rows)
	{
        ...
        if(is_array($rows))
        {
			...
			foreach($rows as $row2){
				...
				$remark_code =  trim($row2['remark_code']);
				...
				$fids_type = (int) ($airport_dest == $airport_base)? 0 : 1; //Added (int) by Tonjaw
				/*if($fids_type == "WAAA") {
					$fids_type = 0;
				} else {
					$fids_type = 1;
				...
				if(empty($total_data) && $fids_type == 1) { // Added || empty() by Tonjaw
					...
					$sql_ary = array(
						...
						'fids_remark'		=> $remark,
						'fids_type' 		=> $fids_type,
						'fids_lastupdate'	=> date('Y-m-d H:i:s'),
						//'airport_fids_update_id'	=> (int) $id,
						'fids_timestamp' 		=> $row2['new_schedule'], //(int) $fids_timestamp,
					...
					// if($fids_flight != ''){
						...
						if($fids_flight=='JT777') echo $sql_insert.' \r\n ';
						$send_notif = true;
						
						/*if(in_array($fids_flight, $flight_strange) )
                                                {
                                                    echo "\nTRUE '$fids_flight'#$fids_type#\n";
                                                }*/
				...
				else 
				{
					...
					//$sql_current_remark = "SELECT fids_remark FROM " . AIRPORT_FIDS_TABLE . " WHERE fids_airline_code_3 = '".$nopen."'";
					//$result_current_remark = $db->sql_query($sql_current_remark);
					//$current_remark = $db->sql_fetchfield('fids_remark');
				 	
					//if(strtoupper($current_remark) != strtoupper($remark)) {
						$sql_ary = array(
							///'fids_flight' 		=> (string) $fids_flight,
							// 'airport_id'		=> (int) $airport_id,
							///'fids_airline_code'	=> (string) $airline_code,
							...
							'fids_lastupdate'	=> date('Y-m-d H:i:s'), //(int) time(),
							'fids_timestamp' 	=> $row2['new_schedule'], //(int) $fids_timestamp,
							///'fids_airline_code_3'	=> (string) $airline_code_3,
							'fids_lastupdate_microtime'	=> $micro_time,
							 'fids_jdate'	=> $jd,
						...
						//if($remark == "Boarding" || $remark == "Last Call" || $remark == "Second Call") echo 'Query update: '.$sql_update.' \r\n ';	
						$send_notif = true;
					//}
				...
				if($send_notif) {
					$ch2 = curl_init();
					curl_setopt($ch2, CURLOPT_URL, $config['API_server']."fis.php?mode=flight_remark&code=".$config['fids_airport_code_default']."&flight=".$fids_flight."&remark=".strtoupper($remark_code)."&jd=".$jd);
				...
				unset($fids_type);      // Added by Tonjaw
				unset($result_check);   // Added by Tonjaw
			}
        }				
    }	
	...
	*/
	$end_start = "______End: ".date("Y-m-d H:i:s")."______\n";
        fwrite($file,$end_start);
	fclose($file);
	echo "\nSLEEP 5...";
	sleep(5);
