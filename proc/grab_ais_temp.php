...
$flight_pool = array("GA659","GA609","GA660", "GA651","GA605","GA608", "GA630","GA643","GA678","GA655","GA621","QG306","QG251","QG778","QG307","QG332","QG349",
"ID6182","ID7708","ID6284","ID6230","ID6285","ID6231","ID6183",
"IW1300","IW1314","IW1869","IW1326","IW1204","IW1220","IW1308",
"JT992","JT786","JT778","JT104","JT852","JT793","JT662","JT841","JT641","JT883","JT797","JT994","JT707","JT676","JT745","JT892","JT787","JT777","JT740","JT780",
...
//	echo 'name: ' . $string['Flight']['Contents']['Content'][0]['name']. '<br>';exit;
	...
	foreach($string as $rows)
	{
		...
		foreach($rows as $row2){
                    ...
                    $airport_dest =  $row2['airport_dest'];
                    $origin =  $row2['origin'];
                    $airport_ori =  $row2['airport_ori'];
...
//			echo $new_time;exit();
			...
			// } else if($row['airport_fids_update_opr'] == 'update' || $row['airport_fids_update_opr'] == 'chg') {
					...
					if($fids_flight == 'IW1300')
					{
						echo "FOUND IW \n"; exit;
						$sql_temp = "SELECT fids_flight AS temp_flight FROM airport_fids_temp WHERE fids_remark='$status' AND fids_flight='$fids_flight'";
						$result_temp = $db->sql_query($sql_temp);
 			                	$temp_flight = $db->sql_fetchfield('temp_flight');
						$db->sql_freeresult($result_temp);
						echo "temp_flight: $temp_flight\n"; exit;
					...
					}elseif( $fids_flight == 'JT807' )
					{
						//echo "FOUND \n"; exit;
                                                $sql_temp = "SELECT fids_flight AS temp_flight FROM airport_fids_temp WHERE fids_flight='JT807' AND fids_remark='$status'";
                                                $result_temp = $db->sql_query($sql_temp);
                                                $temp_flight = $db->sql_fetchfield('temp_flight');
                                                $db->sql_freeresult($result_temp);
