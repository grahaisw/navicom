...
$sql = "SELECT d.galleries_image, d.galleries_image_enabled, d.galleries_clip, d.galleries_clip_enabled, 
	t.translation_title AS title, t.translation_description AS description  
    FROM " . GALLERIES_TABLE . " d 
    JOIN " . GALLERY_TRANSLATIONS_TABLE . " t ON t.galleries_id = d.galleries_id 
    WHERE t.language_id= '" . $lang_id . "' AND d.galleries_enabled = 1  
    ORDER BY d.galleries_order";
...
while ($row = $db->sql_fetchrow($result))
{
	...
	'rec1'			=> $rec1,
	'image'			=> $row['galleries_image'],
	'image_enable'	=> $row['galleries_image_enabled'],
	'clip'			=> $row['galleries_clip'],
	'clip_enable'	=> $row['galleries_clip_enabled'],
	'title'			=> $row['title'],
