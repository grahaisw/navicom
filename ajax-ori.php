...
if($mod == "scheduled_promo") {
	...
	*/
	...
	// 	if($config['notification_in_all_gates']) {
			...
			// 	JOIN " . AIRPORT_FLIGHT_STATUS_TABLE . " f ON upper(a.fids_remark) = upper(f.airport_flight_status_remark) 
			// 	WHERE airport_flight_status_display_on_tv = 1 AND fids_lastupdate_microtime > ".$fids_lastupdate_microtime." 
		...
		// } else {
			$sql2 = "SELECT airport_flight_status_display_mode, airport_flight_status_priority, airport_flight_status_remark, fids_flight, fids_type, fids_city, fids_terminal, fids_gate, fids_time, fids_id, fids_airline_code, r.room_id, airport_flight_status_duration, airport_flight_status_display_period, airport_flight_status_ended_in, a.fids_lastupdate, airport_fids_update_id, fids_gate_old, f.airport_flight_status_remark_id	
				...
				JOIN " . AIRPORT_FLIGHT_STATUS_TABLE . " f ON upper(a.fids_remark) = upper(f.airport_flight_status_remark) 
				WHERE n.node_ip = '158.140.187.232' AND airport_flight_status_display_on_tv = 1 AND a.fids_lastupdate_microtime > ".$fids_lastupdate_microtime."
		...
		while($row = $db->sql_fetchrow($result2)) {		
			...
			$sql_update = "UPDATE " . NODES_TABLE . " SET fids_flag = 0, fids_lastupdate = ".time().", fids_lastupdate_microtime = ".$micro_time." WHERE node_ip = '".$session->ip."'";
