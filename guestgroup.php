...
$sql = "SELECT guest_groups_info_id, guest_groups_info_logo, guest_groups_info_title, guest_groups_info_type
	...
	WHERE guest_groups_info_enabled = 1 AND guest_groups_code = ".$guests_name[0]['group']."";
...
{
	if($data['guest_groups_info_type'] == 'text') {
		$content = prepare_message($row['content']);
	} else if($data['guest_groups_info_type'] == 'image') {
...
$template->assign_vars(array(
	...
	'S_CONTENT_IMAGE'	=> ($data['guest_groups_info_type']=='image') ? 1 : 0,
	'S_CONTENT_TEXT'	=> ($data['guest_groups_info_type']=='text') ? 1 : 0,
