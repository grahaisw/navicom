...
var media = {
    channelListObject: [],
    channelIndex: 0,
	//channelIndex: $("#divCurrentChannelOrder").val(),
    focusIndex: 0,
    controlIndex: 0,
    fullScreen: false,
...
mediaPlayer.prototype.initCreate = function () {
    ...
    for (var i = 0; i < media.channelObjlen; i++) {
        if (i == 0) {
            _div += "<div id='divChannelListItem_" + i + "' cItem='" + i + "' onmouseover='media.JS_MouseOverFunction(this," + i + ")' class='channelFocus spanChannlItem divChannelItems' ><img src='" + channelListArray[i][2] + "' width='136px' ></div>";
        } else {
            _div += "<div id='divChannelListItem_" + i + "' cItem='" + i + "' onmouseover='media.JS_MouseOverFunction(this," + i + ")' class='spanChannlItem divChannelItems' ><img src='" + channelListArray[i][2] + "' width='136px' ></div>";
        };
    };
...
mediaPlayer.prototype.Fn_ChannelListDataBind = function (val) {
    for (var i = 0; i < media.channelObjlen; i++) {
        if (channelListArray[i + val] != undefined) {
            $(media.channelListObject[i]).attr("cItem", (i + val)).html("<img src='" + channelListArray[i + val][2] + "' width='136px' >");
        } else {
...
mediaPlayer.prototype.Fn_PlayLoadSRC = function (sucess) {
    try {
		...
		var param = {
			...
			"ip":channelListArray[media.channelIndex][1],
			"port":1234,
		...
		$("#divCurrentChannelName,#divCurrentChannelNameFullScreen1").html(media.Fn_ValidateChannel(channelListArray[media.channelIndex], true));
        ...
        //$("#divCurrentChannelDescFullScreen").html('<img src="' + channelListArray[media.channelIndex][2] + '" />');
...
media.JS_NumericValidation = function(key) {
...
media.JS_ChannelSwitchByIndex = function(key) {	
    clearTimeout(media.TimerEntry);
    var index = media.JS_NumericValidation(key);	
    if (index) {
        index = index - 1;	
        media.TimerEntry = setTimeout(function() {	
            location.href = "tv_full.php?ch=" + index;
