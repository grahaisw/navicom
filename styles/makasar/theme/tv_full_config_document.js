...
media.bannerTimeoutValue = 10000;
...
media.handler = function (e) {
    ...
    switch (key) {
        case keyCodes.KEY_up:
            var gid = $("#gid").val();
			var total = channelListArray.length;
			var index = $("#divCurrentChannelIndex").val(); 
			if(index < (total-1)) {
				index++;
			} else if(index == (total-1)) {
				index = 0;
			}
            location.href= 'tv_full.php?gid='+gid+'&ch=' + channelListArray[index][5];
            break;
        ...
        case keyCodes.KEY_enter:
			var gid = $("#gid").val();
			//location.href= 'tv_channel.php?mode=ch&gid=' + gid;
        ...
        case keyCodes.KEY_8:
        case keyCodes.KEY_9:
            media.JS_ChannelFullscreenSwitchByIndex(key);
			e.preventDefault();
...
media.JS_SliderOpen = function () {
    $("#divFullScreenControl").stop().animate({ top: "510px" }, "fast");
    media.slide = true;
};
...
media.JS_ChannelFullscreenSwitchByIndex = function(key) {	
    clearTimeout(media.TimerEntry);
    var index = media.JS_NumericValidation(key);	
    if (index) {
        index = index - 1;	
		var idx = $("#divCurrentChannelIndex").val();
		$("#divCurrentChannelIndex").val(index);
        media.TimerEntry = setTimeout(function() {	
            if (Number(index) != idx) {	
                var serlength = media._object.channelListLength;
