        ...
        return confirm('{L_CONFIRM_DELETE}');
			...
			<!-- IF S_USER -->
			  ...
			  <!-- IF user.S_ROW_COUNT is even --><tr class="bg1"><!-- ELSE --><tr class="bg2"><!-- ENDIF -->
			    ...
			    <input type="checkbox" name="mark_{user.S_UID}" {user.V_ENABLED}/></td>
			    <td style="width: 5%" align="center"><a href="{user.U_UPDATE}" rel="facebox"><img src="{user.ICON_PATH}/icon_edit.gif" alt="{user.L_UPDATE}" title="{user.L_UPDATE}" /></a></td>
			    <!-- ELSE -->
			    <td>{user.ENABLED}</td>
			    <!-- ENDIF -->
			    <!-- IF S_DELETE -->
			    <td style="width: 5%" align="center"><a href="{user.U_DELETE}" id="DeleteLink"><img src="{user.ICON_PATH}/icon_delete.gif" alt="{user.L_DELETE}" title="{user.L_DELETE}" /></a></td>
			    <!-- ENDIF -->
			    <!-- IF S_PRIVILEDGE --><td><a href="{user.U_PRIVILEDGE}">{user.L_PRIVILEDGE}</a></td>
			    <!-- ENDIF -->
			  </tr>
			<!-- END user -->
			<!-- ELSE -->
