	...
	<style type="text/css">
		.hilang{
			...
			<tr>
			  ...
			  <th class="hilang">{L_NAME}</th>
			  <th class="hilang">{L_GROUP}</th>
			  <th class="hilang">{L_ALLOW_ADS}</th>
			  <th>{L_ENABLED}</th>
			...
			<!-- IF S_MUSIC -->
			  ...
			  <!-- IF music.S_ROW_COUNT is even --><tr class="bg1"><!-- ELSE --><tr class="bg2"><!-- ENDIF -->
			    ...
			    <td class="hilang"><strong><a href="{music.U_NAME}">{music.NAME}</a></strong>
				<br/>{music.L_CAST}: {music.S_CAST}
				<br/>{music.L_DIRECTOR}: {music.S_DIRECTOR}</td>
			    <td class="hilang">{music.GROUP}<input type="hidden" name="music_id[]" value="{music.S_MID}"/></td>
			    <td class="hilang">{music.ALLOW_ADS}</td>
