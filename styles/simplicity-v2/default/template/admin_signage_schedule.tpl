        ...
        return confirm('{L_CONFIRM_DELETE}');
			...
			<!-- IF S_SIGNAGES -->
			  ...
			  <!-- IF signage.S_ROW_COUNT is even --><tr class="bg1"><!-- ELSE --><tr class="bg2"><!-- ENDIF -->
			    <td>{signage.NAME}</td>
			    <td>{signage.REGION_GROUP_NAME}</td>
                <td>{signage.START}</td>
			    <td>{signage.END}</td>
			    <td>{signage.PLAYLIST}</td>
			    <!-- IF S_ADD_UPDATE --><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_{signage.S_RID}" {signage.V_FULLSCREEN}/><label>&nbsp;</label></td>
			    <!-- ELSE -->
			    <td>{signage.FULLSCREEN}</td>
			    <!-- ENDIF -->
                <!-- IF S_ADD_UPDATE --><td style="width: 5%" align="center">
