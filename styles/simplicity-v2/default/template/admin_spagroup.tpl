        ...
        return confirm('{L_CONFIRM_DELETE}');
			...
			<!-- IF S_GROUPS -->
			  ...
			  <!-- IF group.S_ROW_COUNT is even --><tr class="bg1"><!-- ELSE --><tr class="bg2"><!-- ENDIF -->
			    <td><img src="{group.THUMBNAIL}" alt="{group.NAME}" width="60"></td>
			    <td><strong>{group.NAME}</strong><br/>{group.DESCRIPTION}
			    <input type="hidden" name="group_id[]" value="{group.S_GID}"/></td>
			    <td>{group.ORDER}</td>
			    <td>{group.ALLOW_ADS}</td>
