        ...
        return confirm('{L_CONFIRM_DELETE}');
			...
			<!-- IF S_GUEST_GROUP -->
			  ...
			  <!-- IF group.S_ROW_COUNT is even --><tr class="bg1"><!-- ELSE --><tr class="bg2"><!-- ENDIF -->
			    ...
			    <input type="checkbox" name="mark_{group.S_DID}" {group.V_ENABLED}/><label>&nbsp;</label></td>
			    <!-- ELSE -->
			    <td>{group.ENABLED}</td>
			    <!-- ENDIF -->
			    <!-- IF S_ADD_UPDATE -->
			    <td style="width: 5%" align="center"><a href="{group.U_UPDATE}"><img src="{group.ICON_PATH}/edit.png" alt="{group.L_UPDATE}" title="{group.L_UPDATE}" /></a></td>
			    <!-- ENDIF -->
			    <!-- IF S_DELETE -->
			    <td style="width: 5%" align="center"><a href="{group.U_DELETE}" id="DeleteLink"><img src="{group.ICON_PATH}/delete.png" alt="{group.L_DELETE}" title="{group.L_DELETE}" /></a></td>
			    <!-- ENDIF -->
			  </tr>
			<!-- END group -->
			<!-- ENDIF -->
