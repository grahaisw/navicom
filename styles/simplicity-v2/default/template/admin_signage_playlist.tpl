        ...
        return confirm('{L_CONFIRM_DELETE}');
			...
			<!-- IF S_SIGNAGES -->
			  ...
			  <!-- IF signage.S_ROW_COUNT is even --><tr class="bg1"><!-- ELSE --><tr class="bg2"><!-- ENDIF -->
			    <td><strong><a href="{signage.U_DETAIL}">{signage.NAME}</a></strong></td>
			    <td>{signage.DESCRIPTION} <input type="hidden" name="playlist_id[]" value="{signage.S_RID}"/></td>
                <td>{signage.TYPE}</td>
				
                <!--<td>{signage.DURATION}</td>-->
			    ...
			    <!-- ENDIF -->
                <!-- IF S_ADD_UPDATE --><!--<td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_{signage.S_RID}" {signage.V_LOOP}/></td>-->
			    <!-- ELSE -->
			    <!--<td>{signage.LOOP}</td>-->
			    <!-- ENDIF -->
			    <!-- IF S_ADD_UPDATE -->
