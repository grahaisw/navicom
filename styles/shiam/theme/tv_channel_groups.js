...
mediaPlayer.prototype.initCreate = function () {
    ...
    for (var i = 0; i < media.channelObjlen; i++) {
        if (i == 0) {
            _div += "<div id='divChannelListItem_" + i + "' cItem='" + i + "' onmouseover='media.JS_MouseOverFunction(this," + i + ")' class='channelFocus spanChannlItem divChannelItems' ><img src='" + channelListArray[i][2] + "'></div>";
        } else {
            _div += "<div id='divChannelListItem_" + i + "' cItem='" + i + "' onmouseover='media.JS_MouseOverFunction(this," + i + ")' class='spanChannlItem divChannelItems' ><img src='" + channelListArray[i][2] + "'></div>";
        };
    };
...
mediaPlayer.prototype.Fn_ChannelListDataBind = function (val) {
    for (var i = 0; i < media.channelObjlen; i++) {
        if (channelListArray[i + val] != undefined) {
            $(media.channelListObject[i]).attr("cItem", (i + val)).html("<img src='" + channelListArray[i + val][2] + "' >");
        } else {
