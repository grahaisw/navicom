...
media.handler = function (e) {
    ...
    switch (key) {
        case keyCodes.KEY_up:
			if(media._object.currentArea == "iframe") {
				//$("#order").focus();
				 $(".order").focus();
			} else {
				if (!media.fullScreen) {
        ...
        case keyCodes.KEY_enter:
			/*$.fancybox.open({
					href : 'http://192.168.0.120/focus.html',
					type : 'iframe',
					padding : 5
				});*/
				//fxm.keyboard.open('savePreferences', 'text', 32);
				//sss_js_keyboardDisplayInlineFunction();
				//sss_js_tdstyle_orange;
...
//window.location='index.php?key=3'  + channelListArray[media.channelIndex][0] ;
