...
*
* psoTFX, Tonjaw Development Team - Completion of file caching, decompilation
...
class template_compile
{
		...
		for ($curr_tb = 0, $tb_size = sizeof($blocks); $curr_tb < $tb_size; $curr_tb++)
		{
			...
			switch ($block_val[1])
			{
				...
				default:
					...
					$compile_blocks[] = (!$no_echo) ? ($trim_check) ? $block_val[0] : '' : ($trim_check) ? $block_val[0] : '';
				break;
			}
		}
		...
		for ($i = 0, $size = sizeof($tokens); $i < $size; $i++)
		{
			...
			switch ($token)
			{
				...
				default:
					...
					{
						$token = ($varrefs[1]) ? $this->generate_block_data_ref(substr($varrefs[1], 0, -1), true, $varrefs[2]) . '[\'' . $varrefs[3] . '\']' : (($varrefs[2]) ? '$this->_tpldata[\'DEFINE\'][\'.\'][\'' . $varrefs[3] . '\']' : '$this->_rootref[\'' . $varrefs[3] . '\']');
					}
	...
	*/
	function compile_write($handle, $data)
	{
		...
		$data = "<?php if (!defined('IN_TONJAW')) exit;" . ((strpos($data, '<?php') === 0) ? substr($data, 5) : ' ?>' . $data);
		...
		if ($fp = @fopen($filename, 'wb'))
		{
			...
			tonjaw_chmod($filename, CHMOD_READ | CHMOD_WRITE);
		}
