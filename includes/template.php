...
class template
{
	...
	*/
	...
	{
		global $tonjaw_root_path, $user, $config;
		
		//echo $tonjaw_root_path . $user['template_path']; exit;
		if (file_exists($tonjaw_root_path . $config['template_path']))
		{
			$this->root = $tonjaw_root_path . $config['template_path'];			
		}
		else
		{
			trigger_error('Template path could not be found: ' . $config['template_path'], E_USER_ERROR);
		}
	...
	*
	function set_custom_template($template_path, $template_name, $fallback_template_path = false)
	{
		...
		$this->cachepath = $tonjaw_root_path . 'cache/ctpl_' . str_replace('_', '-', $template_name) . '_';
	...
	*/
	function display($handle, $include_once = true)
	{
		global $user;
/* disabled by tonjaw
		if (!empty($tonjaw_hook) && $tonjaw_hook->call_hook(array(__CLASS__, __FUNCTION__), $handle, $include_once, $this))
		{
			if ($tonjaw_hook->hook_return(array(__CLASS__, __FUNCTION__)))
			{
				return $tonjaw_hook->hook_return_result(array(__CLASS__, __FUNCTION__));
			}
		}
*/
...
*/
		global $db, $tonjaw_root_path;

		if (!class_exists('template_compile'))
		{
			include($tonjaw_root_path . $config['include_path'] . 'functions_template.' . $phpEx);
		}
/*
...
/*
		...
		if (isset($user->theme['template_storedb']) && $user->theme['template_storedb'])
		{
			...
			else
			{
				...
				// Try to load from filesystem and instruct to insert into the templates table...
				$compile->_tpl_load_file($handle, true);
				return false;
			}
...
*/
	...
	*/
	...
	{
		global $tonjaw_root_path;

		$file = $tonjaw_root_path . $filename;
