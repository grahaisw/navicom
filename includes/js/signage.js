for(var i=0; i<=total_region; i++) {   
    playlist(template, r[i]);
}

window.setInterval("checkDefault('" + template + "')", 10000);
...
function playlist(data, region, is_fullscreen) {  
    ...
    if(region_type == "clip") {	
        ...
        if(data == "default" || data.substring(0,3)) var duration = $("#region"+region+"_content_duration").val();
...
function getContent(data, region) {
    ...
    } else if(data.substring(0,3) == "tpl") {
		var con = $("#region"+region+"_content").val();
	} else {
        var con = data; 
    }
...
function checkDefault(template) {
    $.ajax({
...
function checkEmergency() {
    $.ajax({
        ...
        success: function(response){  
            ...
            var emergency_content = '<img src="media/images/signage/emergency.gif" width="100%" height="100%" /><audio id="media2" type="audio/mp3" src="media/audios/alert.ogg" autoplay loop></audio>';
			//var emergency_content = '<video id="media2" preload="auto" loop="true" autoplay="true" src="media/clips/wi-east-java.mp4"></video>';
    ...
    //if(current == stoptime) {
        $.ajax({
            ...
            success: function(response){
                ...
                $("#emergency_stop").val("");
				$("#urgency_id").val("");
				$("#myVideo").attr('src',videoSource);
				$("#region5").append('<marquee scrollamount="10" loop="" scrolldelay="3">'+text+'</marquee>');
