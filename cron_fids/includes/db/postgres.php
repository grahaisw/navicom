...
if (!class_exists('tonjaw_error_collector'))
{
	include('includes/error_collector.php');
}
...
class dbal_postgres extends dbal
{
	...
	* Connect to server
	*/
	function sql_connect($sqlserver, $sqluser, $sqlpassword, $database, $port = false, $persistency = false, $new_link = true)
	{
		$connect_string = '';
		...
		{	
			...
			$collector = new tonjaw_error_collector;
			$collector->install();
		...
		$this->connect_error = $collector->format_errors();echo $this->connect_error;
		return $this->sql_error('');
	}
	...
	*/
	function sql_nextid()
	{
		...
		{
			//echo '<p>postgres. query_id n last_query_text<br/>';
			if (preg_match("/^INSERT[\t\n ]+INTO[\t\n ]+([a-z0-9\_\-]+)/is", $this->last_query_text, $tablename))
			{
				$query = "SELECT currval('" . $tablename[1] . "_seq') AS last_value";
				//$query = "SELECT currval('pages_page_id_seq') AS last_value";
				...
				if (!$temp_q_id)
				{
				    //echo 'temp_q_id empty<br/>';
					return false;
				}

				$temp_result = @pg_fetch_assoc($temp_q_id, NULL);
				@pg_free_result($query_id);
    //echo 'temp_result: ' . $temp_result['last_value'] . '<br/>';
