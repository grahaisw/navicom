...
$sql = "SELECT d.wakeup_calls_image, d.wakeup_calls_image_enabled, d.wakeup_calls_clip, d.wakeup_calls_clip_enabled, 
	t.translation_title AS title, t.translation_description AS description  
    FROM " . WAKEUP_CALLS_TABLE . " d 
    JOIN " . WAKEUP_CALL_TRANSLATIONS_TABLE . " t ON t.wakeup_calls_id = d.wakeup_calls_id 
    WHERE t.language_id= '" . $lang_id . "' AND d.wakeup_calls_enabled = 1  
    ORDER BY d.wakeup_calls_order";
...
while ($row = $db->sql_fetchrow($result))
{
	...
	'rec1'			=> $rec1,
	'image'			=> $row['wakeup_calls_image'],
	'image_enable'	=> $row['wakeup_calls_image_enabled'],
	'clip'			=> $row['wakeup_calls_clip'],
	'clip_enable'	=> $row['wakeup_calls_clip_enabled'],
	'title'			=> $row['title'],
