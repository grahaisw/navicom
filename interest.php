...
$sql = "SELECT d.interest_image, d.interest_image_enabled, d.interest_clip, d.interest_clip_enabled, 
	t.translation_title AS title, t.translation_description AS description  
    FROM " . INTERESTS_TABLE . " d 
    JOIN " . INTEREST_TRANSLATIONS_TABLE . " t ON t.interest_id = d.interest_id 
    WHERE t.language_id= '" . $lang_id . "' AND d.interest_enabled = 1  
    ORDER BY d.interest_order";
...
while ($row = $db->sql_fetchrow($result))
{
	...
	'rec1'			=> $rec1,
	'image'			=> $row['interest_image'],
	'image_enable'	=> $row['interest_image_enabled'],
	'clip'			=> $row['interest_clip'],
	'clip_enable'	=> $row['interest_clip_enabled'],
	'title'			=> $row['title'],
