...
$sql = "SELECT d.doctors_image, d.doctors_image_enabled, d.doctors_clip, d.doctors_clip_enabled, 
	t.translation_title AS title, t.translation_description AS description  
    FROM " . DOCTORS_TABLE . " d 
    JOIN " . DOCTOR_TRANSLATIONS_TABLE . " t ON t.doctors_id = d.doctors_id 
    WHERE t.language_id= '" . $lang_id . "' AND d.doctors_enabled = 1  
    ORDER BY d.doctors_order";
...
while ($row = $db->sql_fetchrow($result))
{
	...
	'rec1'			=> $rec1,
	'image'			=> $row['doctors_image'],
	'image_enable'	=> $row['doctors_image_enabled'],
	'clip'			=> $row['doctors_clip'],
	'clip_enable'	=> $row['doctors_clip_enabled'],
	'title'			=> $row['title'],
