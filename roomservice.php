...
if ( $pms_config['roomservice_item_in_pos'] )
{
	...
	$response = $pms->get_menu_item();
		
    if($response) {
		$j = 0; //print_r($pms->menu_data); exit;
		foreach ($pms->menu_data as $row1)
		{	
		//echo $pms->menu_data[$j]['menu_id'];
		$sql = "SELECT s.service_id, t.translation_title AS service_name, t.translation_description AS service_description, s.service_price, s.service_thumbnail, s.service_allow_ads, tg.translation_title AS group_name, g.service_group_thumbnail AS group_thumbnail, s.service_code, g.service_group_clip AS group_clip, s.service_currency
			    ...
			    AND tg.language_id= '" . $lang_id . "' AND s.service_code = '" . $pms->menu_data[$j]['menu_id'] . "' AND s.service_enabled = 1 ".$filter;
		//echo $sql.'<br>'; exit;
		...
		//echo $service_id;
		$menu_name = $pms->menu_data[$j]['menu_name'];
		$cat_name = $pms->menu_data[$j]['category_name'];
		$menu_thumbnail = $row['service_thumbnail'];
		$menu_description = $description;
		$menu_price = $pms->menu_data[$j]['price'];
		$menu_code = $pms->menu_data[$j]['menu_id'];
		//$menu_group_thumbnail = $pms_config['roomservice_name_from_pos'] ? $row1['category_id'] : $row['group_thumbnail'];
		$menu_currency = $pms->menu_data[$j]['currency'];
		$tax_info = $pms->menu_data[$j]['tax_info'];
		...
		$service_data[$i] = array(
		  'rec1'		=> $rec1,
		  'id'			=> $row1['service_id'],
		  'code'		=> $row1['service_code'],
		  'name'		=> $row1['service_name'],
		  'thumbnail'		=> $row1['service_thumbnail'],
		  'description'		=> $row1['service_description'],
		  'allow_ads'		=> $row1['service_allow_ads'],
		  'price'		=> $row1['service_price'],
		  'group_name'		=> $row1['group_name'],
		  'group_thumbnail'	=> $row1['group_thumbnail'],
		  'group_clip'		=> $row1['group_clip'],
		  'currency'		=> $row1['service_currency'],
		  'tax_info'		=> '',
		);
	...
	//}
//	print_r($service_data); echo 'c'; 
//exit;
	foreach ($service_data as $item)
	{
	    $thumbnail_path = $tonjaw_root_path.$config['media_path'].$config['image_path'].'fnb/600x400/'.$item['thumbnail'].'.jpg';
	   	//echo $thumbnail_path;
		...
		$template->assign_block_vars('roomservice', array(
		    'REC1'			=> $item['rec1'],
		    'S_TITLE'			=> htmlentities($item['name']),
		    'S_PICTURE'			=> $thumbnail,
		    'S_DESCRIPTION'		=> $item['description'],
		    //'S_URL'			=> ($key) ? $item[$config['tv_source_protocol']] . '?key=' . $room_key : $item[$config['tv_source_protocol']],
		    'S_CATEGORY'		=> $item['group_name'],
		    'S_CATEGORY_THUMBNAIL'	=> $item['group_thumbnail'],
		    'S_PRICE'			=> number_format($item['price']), //prepare_message($item['description']),
		    'S_THUMBNAIL'		=> $thumbnail,
		    'S_CODE'			=> $item['code'],
		    'S_QTY'			=> '1',
		    'L_CURRENCY'		=> $item['currency'],
		    'L_PRICE'			=> $lang['price'],
		    'S_SERVICE_ID'		=> $item['id'],
		    'S_GID'			=> $gid,
		    'S_MODE'			=> $mode,
			'S_KEY'				=> $key,
			'S_TAXINFO'			=> $item['tax_info'],
		));
	...
	$template->assign_vars(array(
	    ...
	    'L_PRICE'				=> $lang['price'],
	    'L_CURRENCY'			=> 'NR',
		'L_PRESS_BLUE_BUTTON' => $lang['press_blue_button'],
	    'L_PAGE_TITLE'			=> empty($service_data[0]['group_name']) ? '' : $service_data[0]['group_name'],
	    'L_PAGE_TITLE_UP'                  => $lang['roomservice'],
	    ...
	    'S_RESV_ID'			=> (empty($guests_name[0]['resv_id'])) ? 0 : $guests_name[0]['resv_id'],
	));
...
{ // $pms_config['roomservice_item_in_pos']
	...
	$template->assign_vars(array(
	    ...
	    //'L_PAGE_TITLE'		=> $service_data[0]['group_name'],
		'L_PAGE_TITLE_UP'                  => $lang['roomservice'],
		'L_PRESS_BLUE_BUTTON' => $lang['press_blue_button'],
	    'S_GRADIENT_THUMBNAIL'	=> $service_data[0]['group_thumbnail'],
	    'S_GROUP_CLIP'		=> $service_data[0]['group_clip'],
...
if ( empty($gid) && $mode !== 'item' )
{
    ...
    while ($row = $db->sql_fetchrow($result))
    {
	$sqlCount = "SELECT COUNT(*) AS total_entries FROM ".SERVICES_TABLE." WHERE service_group_id = '".$row['service_group_id']."' AND service_enabled = 1";
	$db->sql_query($sqlCount);
	$total_data_in_group = $db->sql_fetchfield('total_entries');
	if($total_data_in_group > 0) {
	$group_data[$i] = array(
    ...
    foreach ($group_data as $row)
    {
    	...
    	$thumbnail_path = $tonjaw_root_path.$config['media_path'].$config['image_path'].'fnb/category/600x400/'.$row['thumbnail'].'.jpg';
	   	if(!file_exists($thumbnail_path)) {
	   		$thumbnail = $config['default_thumbnail_roomservice'];
	   	} else {
	   		$thumbnail = $row['thumbnail'];
