...
if($mod == "scheduled_promo") {
	...
	*/
	...
	$sql = "SELECT fids_flag, fids_lastupdate, fids_lastupdate_microtime FROM " . NODES_TABLE . "
			WHERE node_ip = '".$session->ip."'";
	$result = $db->sql_query($sql);
	$fids = $db->sql_fetchrow($result);
	$fids_flag = $fids['fids_flag'];
	$fids_lastupdate = $fids['fids_lastupdate'];
	//$fids_lastupdate = '2019-04-01 13:47:35';
	$fids_lastupdate_microtime = $fids['fids_lastupdate_microtime'];
	if(empty($fids_lastupdate_microtime)) {
		$fids_lastupdate_microtime = 0;
	}
	
	$db->sql_freeresult($result);
	
	if($fids_flag == 1) { // jika ada perubahan status flight
	
	 	if($config['notification_in_all_gates']) { 
			 $sql2 = "SELECT airport_flight_status_display_mode, airport_flight_status_priority, airport_flight_status_remark, fids_flight, fids_type, fids_city, fids_terminal, fids_gate, fids_time, fids_id, fids_airline_code, airport_flight_status_duration, airport_flight_status_display_period, airport_flight_status_ended_in, airport_fids_update_id, fids_gate_old, f.airport_flight_status_remark_id	
			 	FROM " . AIRPORT_FIDS_TABLE . " a 
			 	JOIN " . AIRPORT_FLIGHT_STATUS_TABLE . " f ON upper(a.fids_remark) = upper(f.airport_flight_status_remark) 
			 	WHERE airport_flight_status_display_on_tv = 1 AND fids_lastupdate > '".$fids_lastupdate."' 
			 	ORDER BY fids_lastupdate ASC";
				 //echo $sql2;
		 } else { 
			$sql2 = "SELECT airport_flight_status_display_mode, airport_flight_status_priority, airport_flight_status_remark, fids_flight, fids_type, fids_city, fids_terminal, fids_gate, fids_time, fids_id, fids_airline_code, r.room_id, airport_flight_status_duration, airport_flight_status_display_period, airport_flight_status_ended_in, a.fids_lastupdate, airport_fids_update_id, fids_gate_old, f.airport_flight_status_remark_id	
				FROM " . AIRPORT_FIDS_TABLE . " a 
				JOIN " . ROOMS_TABLE . " r ON fids_gate = r.room_name 
				JOIN " . NODES_TABLE . " n ON n.room_id = r.room_id 
				JOIN " . AIRPORT_FLIGHT_STATUS_TABLE . " f ON upper(a.fids_remark) = upper(f.airport_flight_status_remark) 
				WHERE n.node_ip = '".$session->ip."' AND airport_flight_status_display_on_tv = 1 AND a.fids_lastupdate > '".$fids_lastupdate."'
				ORDER BY a.fids_lastupdate ASC";
		...
		while($row = $db->sql_fetchrow($result2)) {		
			...
			$remark_id = $row['airport_flight_status_remark_id'];
			$flight_no = substr($row['fids_flight'], 0, 2)."-".substr($row['fids_flight'], 2);
			$gate = $row['fids_gate'];
			$time = $row['fids_time'];
			$city = $row['fids_city'];
			...
			if(!empty($row['fids_gate_old'])) {
				$remark = 'Change Gate';
				$remark_id = 'Pindah Pintu';
				
				$sql_update_gate = "UPDATE " . AIRPORT_FIDS_TABLE . " SET fids_gate_old = '' WHERE airport_fids_update_id = '".$airport_fids_update_id."'";
			...
			$sql_update = "UPDATE " . NODES_TABLE . " SET fids_flag = 0, fids_lastupdate = '".date('Y-m-d H:i:s')."', fids_lastupdate_microtime = ".$micro_time." WHERE node_ip = '".$session->ip."'"; 
...
}else if($mod == "check_status_stb") {
	$ip = $_GET['ip'];
	if(checkOnline('http://'.$ip.'/url.html')) {  $status = "ON"; }
	//if(ping($ip)) {  $status = "ON"; }
	//else { $status = "OFF"; }
	/*
	if($ip != '') {
		$fp = @fsockopen($ip, 80, $err, $err_string, 1);
		//$fp = exec('ping "' . $ip . '"');
		if(!$fp) {
			$status = "OFF";
		} else {
...
} else if($mod == "last_channel") {
   /* global $db, $config, $guests_name, $session;
    
    $last_tv_channel_id = $_GET['tv_channel_id'];
    $ip = (isset($_GET['ip'])) ? $_GET['ip'] : $session->ip;
	
    $sql = "UPDATE " . NODES_TABLE . " SET node_last_channel = " . $last_tv_channel_id . " WHERE node_ip = '" . $ip ."'";
    $db->sql_query($sql);
...
function checkOnline($domain) {
	...
	//$response = shell_exec("curl '".$domain."' | tail | grep html");
