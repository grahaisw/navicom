...
$sql = "SELECT d.publicplaces_image, d.publicplaces_image_enabled, d.publicplaces_clip, d.publicplaces_clip_enabled, 
	t.translation_title AS title, t.translation_description AS description  
    FROM " . PUBLICPLACES_TABLE . " d 
    JOIN " . PUBLICPLACE_TRANSLATIONS_TABLE . " t ON t.publicplaces_id = d.publicplaces_id 
    WHERE t.language_id= '" . $lang_id . "' AND d.publicplaces_enabled = 1  
    ORDER BY d.publicplaces_order";
...
while ($row = $db->sql_fetchrow($result))
{
	...
	'rec1'			=> $rec1,
	'image'			=> $row['publicplaces_image'],
	'image_enable'	=> $row['publicplaces_image_enabled'],
	'clip'			=> $row['publicplaces_clip'],
	'clip_enable'	=> $row['publicplaces_clip_enabled'],
	'title'			=> $row['title'],
