    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tr>
			  ...
			  <th><?php echo ((isset($this->_rootref['L_REGION_GROUP_NAME'])) ? $this->_rootref['L_REGION_GROUP_NAME'] : ((isset($user->lang['REGION_GROUP_NAME'])) ? $user->lang['REGION_GROUP_NAME'] : '{ REGION_GROUP_NAME }')); ?></th>
              ...
              <th><?php echo ((isset($this->_rootref['L_PLAYLIST'])) ? $this->_rootref['L_PLAYLIST'] : ((isset($user->lang['PLAYLIST'])) ? $user->lang['PLAYLIST'] : '{ PLAYLIST }')); ?></th>
			...
			<?php if ($this->_rootref['S_SIGNAGES']) {  $_signage_count = (isset($this->_tpldata['signage'])) ? sizeof($this->_tpldata['signage']) : 0;if ($_signage_count) {for ($_signage_i = 0; $_signage_i < $_signage_count; ++$_signage_i){$_signage_val = &$this->_tpldata['signage'][$_signage_i]; if (!($_signage_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>
			    ...
			    <td><?php echo $_signage_val['REGION_GROUP_NAME']; ?></td>
                <td><?php echo $_signage_val['START']; ?></td>
			    <td><?php echo $_signage_val['END']; ?></td>
			    <td><?php echo $_signage_val['PLAYLIST']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_signage_val['S_RID']; ?>" <?php echo $_signage_val['V_FULLSCREEN']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>
			    <td><?php echo $_signage_val['FULLSCREEN']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
