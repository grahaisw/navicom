...
/**
*
* admin/signage/signage_templatedetail.php
*
...
$u_action = $tonjaw_admin_signage_path . 'signage_templatedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	    ...
	    'template_enabled'		=> (int) $enabled_flag,
    );
    
    if ($mode === 'add')
    {
        $sql = 'INSERT INTO ' . SIGNAGE_TEMPLATES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
        $sql = 'UPDATE ' . SIGNAGE_TEMPLATES_TABLE . ' SET ' . 
            $db->sql_build_array('UPDATE', $sql_ary) .
            " WHERE template_id = $nextid";
        //echo $sql; exit;
        $db->sql_query($sql);
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    $sql = 'SELECT * FROM ' . SIGNAGE_TEMPLATES_TABLE . " WHERE template_id=" . (int) $nextid;
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_signage_path}signage_templatedetail.$phpEx", "mode=add") . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
...
switch($mode) {
    ...
    case 'add':
        ...
        $template->assign_vars(array(
            'S_FORM'		=> '1',
            'S_NAME'	    => $data['template_name'],
            'S_DESCRIPTION'	=> $data['template_description'],
            'S_ENABLED'		=> ($data['template_enabled'])? 'checked' : '',
            'S_FORM_TOKEN'	=> $s_hidden_fields,
        ));
        break;
    ...
    case 'detail' :
        ...
        $template->assign_vars(array(
            'S_DETAIL'		=> '1',
            'S_NAME'	    => $data['template_name'],
            'S_PREVIEW'	    => $tonjaw_root_path.$config['image_signage_path'].$data['template_name'].'.png',
            'S_DESCRIPTION'	=> $data['template_description'],
            'S_ENABLED'		=> ($data['template_enabled'])? 'Yes' : 'No',
        ));
        break;
...
$template->set_filenames(array(
	'body' => 'admin_signage_templateform.tpl',
));
