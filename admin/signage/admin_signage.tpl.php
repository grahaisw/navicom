    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tr>
			  ...
			  <th><?php echo ((isset($this->_rootref['L_SIGNAGE'])) ? $this->_rootref['L_SIGNAGE'] : ((isset($user->lang['SIGNAGE'])) ? $user->lang['SIGNAGE'] : '{ SIGNAGE }')); ?></th>
              <th><?php echo ((isset($this->_rootref['L_REGION'])) ? $this->_rootref['L_REGION'] : ((isset($user->lang['REGION'])) ? $user->lang['REGION'] : '{ REGION }')); ?></th>
              <th><?php echo ((isset($this->_rootref['L_DEFAULT_SOURCE'])) ? $this->_rootref['L_DEFAULT_SOURCE'] : ((isset($user->lang['DEFAULT_SOURCE'])) ? $user->lang['DEFAULT_SOURCE'] : '{ DEFAULT_SOURCE }')); ?></th>
              <!--<th><?php echo ((isset($this->_rootref['L_DEFAULT_TYPE'])) ? $this->_rootref['L_DEFAULT_TYPE'] : ((isset($user->lang['DEFAULT_TYPE'])) ? $user->lang['DEFAULT_TYPE'] : '{ DEFAULT_TYPE }')); ?></th>-->
              <!--<th><?php echo ((isset($this->_rootref['L_DEFAULT_SOURCE'])) ? $this->_rootref['L_DEFAULT_SOURCE'] : ((isset($user->lang['DEFAULT_SOURCE'])) ? $user->lang['DEFAULT_SOURCE'] : '{ DEFAULT_SOURCE }')); ?></th>-->
			...
			<?php if ($this->_rootref['S_SIGNAGES']) {  $_signage_count = (isset($this->_tpldata['signage'])) ? sizeof($this->_tpldata['signage']) : 0;if ($_signage_count) {for ($_signage_i = 0; $_signage_i < $_signage_count; ++$_signage_i){$_signage_val = &$this->_tpldata['signage'][$_signage_i]; if (!($_signage_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>
			    <td><?php echo $_signage_val['NAME']; ?></td>
			    <td><?php echo $_signage_val['SIGNAGE']; ?></td>
                <td><?php echo $_signage_val['REGION']; ?></td>
			    <td><?php echo $_signage_val['DEFAULT_SOURCE']; ?></td>
			    <!--<td><?php echo $_signage_val['DEFAULT_TYPE']; ?></td>-->
                <!--<td><?php echo $_signage_val['DEFAULT_SOURCE']; ?></td>-->
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?>
