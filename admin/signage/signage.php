...
$signage_data = array();
$signage_count = 0;
$start = view_region_grouping($signage_data, $signage_count);
...
foreach ($signage_data as $row)
{
	...
	'NAME'			=> $row['name'],
	'SIGNAGE'	    => $row['signage_name'],
	'REGION'	    => $row['region_name'],
    'PLAYLIST'	    => $row['playlist_name'],
    'DEFAULT_TYPE'	=> $row['default_type'],
    'DEFAULT_SOURCE'	=> ($row['playlist_id'] > 0) ? 'Playlist: '.$row['playlist_name'] : $row['default_source'],
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_signage_path}signage_detail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_signage_path}signage.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'L_DEFAULT_TYPE'	=> $adm_lang['default_type'],
    'L_DEFAULT_SOURCE'	=> $adm_lang['default_source'],
    'L_SIGNAGE'		    => $adm_lang['signage'],
    'L_REGION'			=> $adm_lang['region'],
    'L_SUBMIT'			=> $adm_lang['submit'],
    'S_SIGNAGES'		=> ($signage_count > 0),
    'L_PLAYLIST'		=> $adm_lang['default_playlist'],
));

$template->set_filenames(array(
	'body' => 'admin_signage.tpl',
));
