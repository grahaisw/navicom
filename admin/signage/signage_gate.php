...
/**
*
* admin/signage/signage_gate.php
*
...
$u_action = append_sid("{$tonjaw_admin_signage_path}signage_gate.$phpEx", "mode=update");
...
$signage_data = array();
$signage_count = 0;
$start = view_gates($signage_data, $signage_count);
...
if ($mode === 'update')
{
	...
	$sql = 'UPDATE ' . TARGET_GATES_TABLE . ' 
	  SET target_gate_enabled=' . (string) $mark[$i] ."
	  WHERE target_gate_id = '" . $signage_id[$i] . "'";
	//echo $sql . "<br/>"; 
...
foreach ($signage_data as $row)
{
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_signage_path}signage_gatedetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_signage_path}signage_gate.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_DELETE'		=> $adm_lang['delete'],
	'ICON_PATH'		=> $tonjaw_root_path . $config['imageset_path'],
    'U_DETAIL'		=> append_sid("{$tonjaw_admin_signage_path}signage_gatedetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    ));
}
...
$template->assign_vars(array(
    ...
    'U_ADD'			    => append_sid("{$tonjaw_admin_signage_path}signage_gatedetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			    => $adm_lang['add'],
