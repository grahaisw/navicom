...
/**
*
* admin/signage/signage_schedule.php
*
...
$signage_data = array();
$signage_count = 0;
$start = view_content_schedule($signage_data, $signage_count);
...
foreach ($signage_data as $row)
{
	...
	'REGION_GROUP_NAME'	    => $row['region_group_name'],
	...
	'V_FULLSCREEN'	=> ($row['fullscreen'])? 'checked' : '',
	'FULLSCREEN'	=> !empty($row['signage_content_schedule_fullscreen']) ? 'Yes' : 'No',
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_signage_path}signage_scheduledetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_signage_path}signage_schedule.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->set_filenames(array(
	'body' => 'admin_signage_schedule.tpl',
));
