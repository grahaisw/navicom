    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tr>
              ...
              <th><?php echo ((isset($this->_rootref['L_TEMPLATE'])) ? $this->_rootref['L_TEMPLATE'] : ((isset($user->lang['TEMPLATE'])) ? $user->lang['TEMPLATE'] : '{ TEMPLATE }')); ?></th>
              <th><?php echo ((isset($this->_rootref['L_ZONES'])) ? $this->_rootref['L_ZONES'] : ((isset($user->lang['ZONES'])) ? $user->lang['ZONES'] : '{ ZONES }')); ?></th>
              <th><?php echo ((isset($this->_rootref['L_ROOMS'])) ? $this->_rootref['L_ROOMS'] : ((isset($user->lang['ROOMS'])) ? $user->lang['ROOMS'] : '{ ROOMS }')); ?></th>
			...
			<?php if ($this->_rootref['S_SIGNAGES']) {  $_signage_count = (isset($this->_tpldata['signage'])) ? sizeof($this->_tpldata['signage']) : 0;if ($_signage_count) {for ($_signage_i = 0; $_signage_i < $_signage_count; ++$_signage_i){$_signage_val = &$this->_tpldata['signage'][$_signage_i]; if (!($_signage_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>
			    ...
			    <td><?php echo $_signage_val['DESCRIPTION']; ?><input type="hidden" name="signage_id[]" value="<?php echo $_signage_val['S_RID']; ?>"/></td>
                <td><?php echo $_signage_val['TEMPLATE']; ?></td>
                <td><?php echo $_signage_val['ZONES']; ?></td>
                <td><?php echo $_signage_val['ROOMS']; ?></td>
