...
$u_action = append_sid("{$tonjaw_admin_signage_path}signage_urgency.$phpEx", "mode=update");
...
$signage_data = array();
$signage_count = 0;
$start = view_urgency($signage_data);
$signage_count = 1;
...
if ($mode === 'update')
{
    ...
    foreach($signage_data as $row)
    {
        ...
        $sql = 'UPDATE ' . SIGNAGE_URGENCIES_TABLE . ' 
          SET signage_urgency_enabled=' . (string) $mark[$i] ."";
        $db->sql_query($sql);
...
foreach ($signage_data as $row)
{
    ...
    'EMERGENCY_TYPE' => $row['emergency_name'],
    'TARGET'         => $target,
	'V_ENABLED'		=> ($row['signage_urgency_enabled'])? 'checked' : '',
	'ENABLED'		=> !empty($row['signage_urgency_enabled']) ? 'Yes' : 'No',
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_signage_path}signage_urgencydetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_signage_path}signage_urgency.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_DELETE'		=> $adm_lang['delete'],
	'ICON_PATH'		=> $tonjaw_root_path . $config['imageset_path'],
    'U_DETAIL'		=> append_sid("{$tonjaw_admin_signage_path}signage_urgencydetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    ));
}
...
$template->assign_vars(array(
    ...
    'U_ADD'			    => append_sid("{$tonjaw_admin_signage_path}signage_urgencydetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			    => $adm_lang['add'],
...
$template->set_filenames(array(
	'body' => 'admin_signage_urgency.tpl',
));
