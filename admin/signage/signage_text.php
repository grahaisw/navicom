...
/**
*
* admin/signage/signage_text.php
*
...
$signage_data = array();
$signage_count = 0;
$start = view_contents($signage_data, $signage_count, 'text');
...
if ($mode === 'update')
{
	...
	$sql = 'UPDATE ' . SIGNAGE_TEXT_TABLE . ' 
	  SET signage_text_enabled 	=' . (string) $mark[$i] ."
	  WHERE signage_text_id = '" . $signage_id[$i] . "'";
...
{
    $sql = "DELETE FROM ".SIGNAGE_TEXT_TABLE." WHERE signage_text_id = ".$_GET['id'];
    $db->sql_query($sql);
...
foreach ($signage_data as $row)
{
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_signage_path}signage_textdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_signage_path}signage_text.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_DELETE'		=> $adm_lang['delete'],
	'ICON_PATH'		=> $tonjaw_root_path . $config['imageset_path'],
    'U_DETAIL'		=> append_sid("{$tonjaw_admin_signage_path}signage_textdetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    ));
}
...
$template->assign_vars(array(
    ...
    'U_ADD'			    => append_sid("{$tonjaw_admin_signage_path}signage_textdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			    => $adm_lang['add'],
