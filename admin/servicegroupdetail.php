...
$u_action = $tonjaw_admin_path . 'servicegroupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $allow_ads = request_var('allow_ads', '');
    
    $enabled_flag = $enabled_flag == 'on' ? '1' : '0';
    $allow_ads = $allow_ads == 'on' ? '1' : '0';
    
    $sql_ary = array(
	//'service_group_description'	=> $description,
	'service_group_enabled'		=> (int) $enabled_flag,
	'service_group_order'		=> (int) $order,
	'service_group_allow_ads'	=> (int) $allow_ads,
	'service_group_thumbnail'	=> $thumbnail,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . SERVICE_GROUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . SERVICE_GROUPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE service_group_id = $gid";
	...
	{
	    $sql = 'INSERT INTO ' . SERVICE_GROUP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . SERVICE_GROUP_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'servicegroup.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($gid))
    {
	die('Missing Service Group ID. Cannot update Service Group Table.');
    }
    ...
    $sql = 'SELECT * FROM ' . SERVICE_GROUPS_TABLE . " WHERE service_group_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . SERVICE_GROUP_TRANSLATIONS_TABLE . " WHERE service_group_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}servicegroupdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
    ...
    'S_ORDER'			=> $data['service_group_order'],
    'S_THUMBNAIL'		=> $data['service_group_thumbnail'],
    'S_ENABLED'			=> ($data['service_group_enabled'])? 'checked' : '',
    'S_ALLOW_ADS'		=> ($data['service_group_allow_ads'])? 'checked' : '',
...
$template->set_filenames(array(
	'body' => 'admin_servicegroupform.tpl',
));
