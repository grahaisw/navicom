...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tbody>
			<?php if ($this->_rootref['S_MOVIES']) {  $_movie_count = (isset($this->_tpldata['movie'])) ? sizeof($this->_tpldata['movie']) : 0;if ($_movie_count) {for ($_movie_i = 0; $_movie_i < $_movie_count; ++$_movie_i){$_movie_val = &$this->_tpldata['movie'][$_movie_i]; if (!($_movie_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><a href="<?php echo $_movie_val['U_NAME']; ?>"><img src="<?php echo $_movie_val['THUMBNAIL_PATH']; ?>160x235/<?php echo $_movie_val['THUMBNAIL']; ?>" alt="<?php echo $_movie_val['NAME']; ?>" width="60"></a></td>
			    <td><strong><a href="<?php echo $_movie_val['U_NAME']; ?>"><?php echo $_movie_val['NAME']; ?></a></strong>
				<br/><?php echo $_movie_val['L_CAST']; ?>: <?php echo $_movie_val['S_CAST']; ?>

				<br/><?php echo $_movie_val['L_DIRECTOR']; ?>: <?php echo $_movie_val['S_DIRECTOR']; ?></td>
			    <td><?php echo $_movie_val['GROUP']; ?><input type="hidden" name="movie_id[]" value="<?php echo $_movie_val['S_MID']; ?>"/></td>
			    <td><?php echo $_movie_val['ALLOW_ADS']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_movie_val['S_MID']; ?>" <?php echo $_movie_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_movie_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_movie_val['U_UPDATE']; ?>"><img src="<?php echo $_movie_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_movie_val['L_UPDATE']; ?>" title="<?php echo $_movie_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_movie_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_movie_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_movie_val['L_DELETE']; ?>" title="<?php echo $_movie_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
