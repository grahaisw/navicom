...
$u_action = append_sid("{$tonjaw_admin_path}laundry.$phpEx", "mode=update");
...
$start = grab_directory_grouping($directory_data, $directory_count, "laundry");
//print_r($start); exit;
...
if ($mode === 'update')
{
    ...
    $mid    = (isset($_REQUEST['laundry_id'])) ? request_var('laundry_id', array('0')) : array();
    ...
    $sql = 'UPDATE ' . LAUNDRY_TABLE . ' 
      SET laundry_enabled=' . (string) $mark[$i] ."
      WHERE laundry_id = '" . $mid[$i] . "'";
...
foreach ($directory_data as $row)
{
    ...
    'S_DID'         => $row['id'],
    'TITLE'         => $row['title'],
    'U_TITLE'       => append_sid("{$tonjaw_admin_path}laundrydetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    ...
    'U_UPDATE'      => append_sid("{$tonjaw_admin_path}laundrydetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_UPDATE'      => $adm_lang['edit'],
    'U_DELETE'      => append_sid("{$tonjaw_admin_path}laundry.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
