    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tr>
			  ...
			  <th><?php echo ((isset($this->_rootref['L_DAILY'])) ? $this->_rootref['L_DAILY'] : ((isset($user->lang['DAILY'])) ? $user->lang['DAILY'] : '{ DAILY }')); ?></th>
			  <th><?php echo ((isset($this->_rootref['L_GLOBAL'])) ? $this->_rootref['L_GLOBAL'] : ((isset($user->lang['GLOBAL'])) ? $user->lang['GLOBAL'] : '{ GLOBAL }')); ?></th>
			...
			<tbody>
			<?php if ($this->_rootref['S_RUNNINGTEXT']) {  $_runningtext_count = (isset($this->_tpldata['runningtext'])) ? sizeof($this->_tpldata['runningtext']) : 0;if ($_runningtext_count) {for ($_runningtext_i = 0; $_runningtext_i < $_runningtext_count; ++$_runningtext_i){$_runningtext_val = &$this->_tpldata['runningtext'][$_runningtext_i]; if (!($_runningtext_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_runningtext_val['ORDER']; ?></td>
				<td><strong><a href="<?php echo $_runningtext_val['U_NAME']; ?>"><?php echo $_runningtext_val['MESSAGE']; ?></a></strong></td>
			    <td><?php echo $_runningtext_val['TARGET']; ?><input type="hidden" name="message_id[]" value="<?php echo $_runningtext_val['S_MID']; ?>"/></td>
			    <td><?php echo $_runningtext_val['START']; ?></td>
			    <td><?php echo $_runningtext_val['END']; ?></td>
			    <td><?php echo $_runningtext_val['DAILY']; ?></td>
			    <td><?php echo $_runningtext_val['GLOBAL']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_runningtext_val['S_MID']; ?>" <?php echo $_runningtext_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_runningtext_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_runningtext_val['U_UPDATE']; ?>"><img src="<?php echo $_runningtext_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_runningtext_val['L_UPDATE']; ?>" title="<?php echo $_runningtext_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_runningtext_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_runningtext_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_runningtext_val['L_DELETE']; ?>" title="<?php echo $_runningtext_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
