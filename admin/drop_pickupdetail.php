...
$u_action = $tonjaw_admin_path . 'drop_pickupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'drop_pickups_image'		=> $image,
	'drop_pickups_image_enabled'	=> (int) $image_enabled,
	'drop_pickups_clip'		=> $clip,
	'drop_pickups_clip_enabled'	=> (int) $clip_enabled,
	'drop_pickups_order'		=> (int) ($order)? $order : '1',
	'drop_pickups_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . DROP_PICKUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . DROP_PICKUPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE drop_pickups_id = $did";
	...
	$drop_pickups_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$drop_pickups_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'drop_pickups_id'		=> (int) $did,
	    'translation_title'		=> (string) $drop_pickups_title,
	    'translation_description'	=> (string) $drop_pickups_content,
	...
	{
	    $sql = 'INSERT INTO ' . DROP_PICKUP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . DROP_PICKUP_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'drop_pickup.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . DROP_PICKUPS_TABLE . " WHERE drop_pickups_id = $did" ;
    ...
    $data_thumbnail = ($data['drop_pickups_image'])? $data['drop_pickups_image'] : '0';
    $data_clip = ($data['drop_pickups_clip'])? $data['drop_pickups_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['drop_pickups_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . DROP_PICKUP_TRANSLATIONS_TABLE . " WHERE drop_pickups_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}drop_pickupdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['drop_pickups_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['drop_pickups_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['drop_pickups_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['drop_pickups_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['drop_pickups_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['drop_pickups_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['drop_pickups_order'],
	    'S_CLIP_ENABLED'	=> ($data['drop_pickups_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['drop_pickups_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['drop_pickups_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}drop_pickupdetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_drop_pickupform.tpl',
));
