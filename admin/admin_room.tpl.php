    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tr>
			  ...
			  <th><?php echo ((isset($this->_rootref['L_ZONES'])) ? $this->_rootref['L_ZONES'] : ((isset($user->lang['ZONES'])) ? $user->lang['ZONES'] : '{ ZONES }')); ?></th>
			  <th><?php echo ((isset($this->_rootref['L_NODES'])) ? $this->_rootref['L_NODES'] : ((isset($user->lang['NODES'])) ? $user->lang['NODES'] : '{ NODES }')); ?></th>
			...
			<tbody>
			<?php if ($this->_rootref['S_ROOMS']) {  $_room_count = (isset($this->_tpldata['room'])) ? sizeof($this->_tpldata['room']) : 0;if ($_room_count) {for ($_room_i = 0; $_room_i < $_room_count; ++$_room_i){$_room_val = &$this->_tpldata['room'][$_room_i]; if (!($_room_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_room_val['NAME']; ?></td>
			    <td><?php echo $_room_val['ZONES']; ?></td>
			    <td><?php echo $_room_val['NODES']; ?></td>
			    <td><?php echo $_room_val['DESCRIPTION']; ?><input type="hidden" name="room_id[]" value="<?php echo $_room_val['S_RID']; ?>"/></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_room_val['S_RID']; ?>" <?php echo $_room_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_room_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_room_val['U_UPDATE']; ?>" rel="facebox"><img src="<?php echo $_room_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_room_val['L_UPDATE']; ?>" title="<?php echo $_room_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_room_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_room_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_room_val['L_DELETE']; ?>" title="<?php echo $_room_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
