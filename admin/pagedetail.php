...
$u_action = $tonjaw_admin_path . 'pagedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $in_menu_flag = request_var('in_menu_flag', '');
    $allow_ads_flag = request_var('allow_ads_flag', '');
    $enabled_flag = request_var('enabled_flag', '');
    
    $clip_enabled = $clip_enabled == 'on' ? '1' : '0';
    $in_menu_flag = $in_menu_flag == 'on' ? '1' : '0';
    $enabled_flag = $enabled_flag == 'on' ? '1' : '0';
    $allow_ads_flag = $allow_ads_flag == 'on' ? '1' : '0';
    
    $sql_ary = array(
	    'page_thumbnail'	=> $upload_thumbnail,
	    'page_clip'		=> $clip,
	    'page_clip_enabled'	=> (int) $clip_enabled,
	    'page_in_menu'	=> (int) $in_menu_flag,
	    'page_enabled'	=> (int) $enabled_flag,
	    'page_allow_ads'	=> (int) $allow_ads_flag,
    );
	...
	$page_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$page_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'page_id'			=> (int) $pid,
	    'page_translation_title'	=> (string) $page_title,
	    'page_translation_content'	=> (string) $page_content,
	...
	{
	    $sql = 'INSERT INTO ' . PAGE_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . PAGE_TRANSLATIONS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_translation) .
	    " WHERE page_translation_id = " .$translation_id;
	}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($pid))
    {
	die('Missing Page ID. Cannot update Pages Table.');
    }
    
    $label = ($mode === 'update') ? $adm_lang['update_item'] : $adm_lang['view_item'];
    
    // Grab page data
    $sql = 'SELECT * FROM ' . PAGES_TABLE . " WHERE page_id = $pid" ;
    ...
    $data_thumbnail = ($data['page_thumbnail'])? $data['page_thumbnail'] : '0';
    $data_clip = ($data['page_clip'])? $data['page_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['image_path'] . $data_thumbnail;
    $clip =  $tonjaw_root_path . $config['media_path'] . $config['clip_path'] . $data_clip;
    
    $sql = 'SELECT * FROM ' . PAGE_TRANSLATIONS_TABLE . " WHERE page_id = $pid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
	...
	$detail_data[$detail['language_id']] = array(
	    'translation_id'		=> $detail['page_translation_id'],
	    'translation_content'	=> $detail['page_translation_content'],
	    'translation_title'		=> $detail['page_translation_title'],
	);
...
$template->assign_vars(array(
    ...
    'L_CLIP_ENABLED'		=> $adm_lang['clip_enabled'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['page_thumbnail'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['page_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['thumbnail'],
	    'V_CLIP_ENABLED'		=> ($data['page_clip_enabled'])? 'checked' : '',
	    'V_IN_MENU'			=> ($data['page_in_menu'])? 'checked' : '',
	    'V_ALLOW_ADS'		=> ($data['page_allow_ads'])? 'checked' : '',
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['page_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_CLIP_FILE'		=> $clip,
	    'S_DETAIL'			=> '1',
	    'S_IN_MENU'			=> ($data['page_in_menu'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ALLOW_ADS'		=> ($data['page_allow_ads'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'			=> ($data['page_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}pagedetail.$phpEx", "mode=update") . '&amp;id=' .$pid . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
