...
$u_action = append_sid("{$tonjaw_admin_path}shop_buffer.$phpEx", "mode=update");
...
if ($mode === 'approve') //process
{
	...
	$values = array(
	    ...
	    'outlet_id'		=> $pms_config['outlet_id'][1]['code'],
	);
    ...
    redirect($config['admin_path'] . 'shop_buffer.' . $phpEx, $sid);
}
...
$start = view_buffers($buffer_data, $buffer_count, $config['shop_buffer_type']);
...
foreach ($buffer_data as $row)
{
    ...
    $url = $tonjaw_admin_path . "shop_buffer_detail.$phpEx?mode=list&sid=$sid&id=" . $row['id'];
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}shop_buffer.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_RECEIVED'		=> append_sid("{$tonjaw_admin_path}shop_buffer.$phpEx", "mode=approve") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}shop_buffer.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
