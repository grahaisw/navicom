...
$u_action = $tonjaw_admin_path . 'informationdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    if ($mode === 'add')
    {
	$sql = 'INSERT INTO ' . INFORMATION_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . INFORMATION_TABLE . " SET " . 
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab tv data
    $sql = 'SELECT * FROM ' . INFORMATION_TABLE . " WHERE info_id = $tid" ;
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}informationdetail.$phpEx", "mode=add") . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
    'L_NAME'			=> $adm_lang['name'],
    'L_ORDER'			=> $adm_lang['order'],
    'L_GROUPNAME'		=> $adm_lang['group_name'],
    'L_NOMOR'			=> $adm_lang['nomor'],
    ...
    'S_NAME'			=> $data['info_nama'],
    'S_NOMOR'			=> $data['info_nomor'],
...
$template->set_filenames(array(
	'body' => 'admin_informationdetail.tpl',
));
