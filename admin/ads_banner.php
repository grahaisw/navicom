...
$u_action = append_sid("{$tonjaw_admin_path}ads_banner.$phpEx", "mode=update");
...
$start = view_ads_banner($ads_data, $ads_count);
...
if ($mode === 'update')
{
	...
	$sql = 'UPDATE ' . ADS_BANNERS_TABLE . ' 
	  SET ads_banner_enabled = ' . (string) $mark[$i] ."
	  WHERE ads_banner_id=" . $nid[$i];
...
{
	$sql = 'DELETE FROM ' . ADS_BANNERS_TABLE . ' WHERE ads_banner_id=' . $_GET['id'];
	$db->sql_query($sql);
...
foreach ($ads_data as $row)
{
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}ads_bannerdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}ads_banner.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
