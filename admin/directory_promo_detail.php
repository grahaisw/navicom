...
$u_action = $tonjaw_admin_path . 'directory_promo_detail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'directory_promo_image'		=> $image,
	'directory_promo_image_enabled'	=> (int) $image_enabled,
	'directory_promo_clip'		=> $clip,
	'directory_promo_clip_enabled'	=> (int) $clip_enabled,
	'directory_promo_order'		=> (int) ($order)? $order : '1',
	'directory_promo_enabled'	=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . DIRECTORY_PROMOS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . DIRECTORY_PROMOS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE directory_promo_id = $did";
	...
	{
	    $sql = 'INSERT INTO ' . DIRECTORY_PROMO_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . DIRECTORY_PROMO_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'directory_promo.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($did))
    {
	die('Missing Directory Promo ID. Cannot update Directory Promos Table.');
    }
    // Grab directory data
    $sql = 'SELECT * FROM ' . DIRECTORY_PROMOS_TABLE . " WHERE directory_promo_id = $did" ;
    ...
    $data_thumbnail = ($data['directory_promo_image'])? $data['directory_promo_image'] : '0';
    $data_clip = ($data['directory_promo_clip'])? $data['directory_promo_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['directory_promo_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . DIRECTORY_PROMO_TRANSLATIONS_TABLE . " WHERE directory_promo_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}directory_promo_detail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['directory_promo_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['directory_promo_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['directory_promo_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['directory_promo_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['directory_promo_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['directory_promo_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['directory_promo_order'],
	    'S_CLIP_ENABLED'	=> ($data['directory_promo_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['directory_promo_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['directory_promo_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}directory_promo_detail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_directory_promo_form.tpl',
));
