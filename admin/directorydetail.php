...
$u_action = $tonjaw_admin_path . 'directorydetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
	...
	'directory_image_enabled'	=> (int) $image_enabled,
	'directory_clip'		=> $clip,
	'directory_clip_enabled'	=> (int) $clip_enabled,
	'directory_order'		=> (int) ($order)? $order : '1',
	'directory_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . DIRECTORIES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . DIRECTORIES_TABLE . " SET " . 
	...
	{
	    $sql = 'INSERT INTO ' . DIRECTORY_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . DIRECTORY_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'directory.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab directory data
    $sql = 'SELECT * FROM ' . DIRECTORIES_TABLE . " WHERE directory_id = $did" ;
    ...
    $sql = 'SELECT * FROM ' . DIRECTORY_TRANSLATIONS_TABLE . " WHERE directory_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}directorydetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['directory_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['directory_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['directory_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['directory_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['directory_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['directory_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['directory_order'],
	    'S_CLIP_ENABLED'	=> ($data['directory_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['directory_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['directory_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}directorydetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
