...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<thead>
			<tr>
			  <th><?php echo ((isset($this->_rootref['L_PAGE_ID'])) ? $this->_rootref['L_PAGE_ID'] : ((isset($user->lang['PAGE_ID'])) ? $user->lang['PAGE_ID'] : '{ PAGE_ID }')); ?></th>
			  <th><?php echo ((isset($this->_rootref['L_TITLE'])) ? $this->_rootref['L_TITLE'] : ((isset($user->lang['TITLE'])) ? $user->lang['TITLE'] : '{ TITLE }')); ?></th>
			  <th><?php echo ((isset($this->_rootref['L_IN_MENU'])) ? $this->_rootref['L_IN_MENU'] : ((isset($user->lang['IN_MENU'])) ? $user->lang['IN_MENU'] : '{ IN_MENU }')); ?></th>
			...
			<tbody>
			<?php if ($this->_rootref['S_PAGE']) {  $_page_count = (isset($this->_tpldata['page'])) ? sizeof($this->_tpldata['page']) : 0;if ($_page_count) {for ($_page_i = 0; $_page_i < $_page_count; ++$_page_i){$_page_val = &$this->_tpldata['page'][$_page_i]; if (!($_page_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_page_val['PAGE_ID']; ?></td>
			    <td><a href="<?php echo $_page_val['U_TITLE']; ?>"><?php echo $_page_val['TITLE']; ?></a></td>
			    <td><?php echo $_page_val['IN_MENU']; ?></td>
			    <td><?php echo $_page_val['ALLOW_ADS']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="hidden" name="page_id[]" value="<?php echo $_page_val['S_PID']; ?>"/>
			    <input type="checkbox" name="mark_<?php echo $_page_val['S_PID']; ?>" <?php echo $_page_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_page_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_page_val['U_UPDATE']; ?>"><img src="<?php echo $_page_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_page_val['L_UPDATE']; ?>" title="<?php echo $_page_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_page_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_page_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_page_val['L_DELETE']; ?>" title="<?php echo $_page_val['L_DELETE']; ?>" /></a>
			    <input type="hidden" name="page_id[]" value="<?php echo $_page_val['S_PID']; ?>"/></td>
			    <?php } ?>
