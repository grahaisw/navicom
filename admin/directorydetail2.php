...
/**
*
* admin/directorydetail2.php
*
* Roberto Tonjaw. Apr 2015
*/
...
$u_action = $tonjaw_admin_path . 'directorydetail2.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'directory2_image'		=> $image,
	'directory2_image_enabled'	=> (int) $image_enabled,
	'directory2_clip'		=> $clip,
	'directory2_clip_enabled'	=> (int) $clip_enabled,
	'directory2_order'		=> (int) ($order)? $order : '1',
	'directory2_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . DIRECTORIES2_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . DIRECTORIES2_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE directory2_id = $did";
	...
	{
	    $sql = 'INSERT INTO ' . DIRECTORY2_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . DIRECTORY2_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'directory2.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($did))
    {
	die('Missing Directory2 ID. Cannot update Directories Table.');
    }
    // Grab directory data
    $sql = 'SELECT * FROM ' . DIRECTORIES2_TABLE . " WHERE directory2_id = $did" ;
    ...
    $data_thumbnail = ($data['directory2_image'])? $data['directory2_image'] : '0';
    $data_clip = ($data['directory2_clip'])? $data['directory2_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['directory_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . DIRECTORY2_TRANSLATIONS_TABLE . " WHERE directory2_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}directorydetail2.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['directory2_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['directory2_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['directory2_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['directory2_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['directory2_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['directory2_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['directory2_order'],
	    'S_CLIP_ENABLED'	=> ($data['directory2_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['directory2_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['directory2_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}directorydetail2.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
