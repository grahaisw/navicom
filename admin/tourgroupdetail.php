...
$u_action = $tonjaw_admin_path . 'tourgroupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
	...
	'tour_group_enabled'	=> (int) $enabled_flag,
	'tour_group_order'	=> (int) $order,
	'tour_group_allow_ads'	=> (int) $allow_ads,
	'tour_group_thumbnail'	=> $thumbnail,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . TOUR_GROUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . TOUR_GROUPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE tour_group_id = $gid";
	...
	{
	    $sql = 'INSERT INTO ' . TOUR_GROUP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . TOUR_GROUP_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'tourgroup.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($gid))
    {
	die('Missing Tour Group ID. Cannot update Tour Group Table.');
    }
    ...
    // Get tour data for updating
    $sql = 'SELECT * FROM ' . TOUR_GROUPS_TABLE . " WHERE tour_group_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . TOUR_GROUP_TRANSLATIONS_TABLE . " WHERE tour_group_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}tourgroupdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
    ...
    'S_ORDER'			=> $data['tour_group_order'],
    'S_THUMBNAIL'		=> $data['tour_group_thumbnail'],
    'S_ENABLED'			=> ($data['tour_group_enabled'])? 'checked' : '',
    'S_ALLOW_ADS'		=> ($data['tour_group_allow_ads'])? 'checked' : '',
