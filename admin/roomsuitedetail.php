...
$u_action = $tonjaw_admin_path . 'roomsuitedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'roomsuites_image'		=> $image,
	'roomsuites_image_enabled'	=> (int) $image_enabled,
	'roomsuites_clip'		=> $clip,
	'roomsuites_clip_enabled'	=> (int) $clip_enabled,
	'roomsuites_order'		=> (int) ($order)? $order : '1',
	'roomsuites_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . ROOMSUITES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . ROOMSUITES_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE roomsuites_id = $did";
	...
	$roomsuites_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$roomsuites_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'roomsuites_id'		=> (int) $did,
	    'translation_title'		=> (string) $roomsuites_title,
	    'translation_description'	=> (string) $roomsuites_content,
	...
	{
	    $sql = 'INSERT INTO ' . ROOMSUITE_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . ROOMSUITE_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'roomsuite.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . ROOMSUITES_TABLE . " WHERE roomsuites_id = $did" ;
    ...
    $data_thumbnail = ($data['roomsuites_image'])? $data['roomsuites_image'] : '0';
    $data_clip = ($data['roomsuites_clip'])? $data['roomsuites_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['roomsuites_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . ROOMSUITE_TRANSLATIONS_TABLE . " WHERE roomsuites_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}roomsuitedetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['roomsuites_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['roomsuites_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['roomsuites_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['roomsuites_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['roomsuites_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['roomsuites_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['roomsuites_order'],
	    'S_CLIP_ENABLED'	=> ($data['roomsuites_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['roomsuites_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['roomsuites_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}roomsuitedetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
