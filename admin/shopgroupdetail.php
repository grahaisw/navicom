...
$u_action = $tonjaw_admin_path . 'shopgroupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
	...
	'shop_group_enabled'	=> (int) $enabled_flag,
	'shop_group_order'	=> (int) $order,
	'shop_group_allow_ads'	=> (int) $allow_ads,
	'shop_group_thumbnail'	=> $thumbnail,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . SHOP_GROUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . SHOP_GROUPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE shop_group_id = $gid";
	...
	{
	    $sql = 'INSERT INTO ' . SHOP_GROUP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . SHOP_GROUP_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'shopgroup.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($gid))
    {
	die('Missing Shop Group ID. Cannot update Shop Group Table.');
    }
    ...
    $sql = 'SELECT * FROM ' . SHOP_GROUPS_TABLE . " WHERE shop_group_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . SHOP_GROUP_TRANSLATIONS_TABLE . " WHERE shop_group_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}shopgroupdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
    ...
    'S_ORDER'			=> $data['shop_group_order'],
    'S_THUMBNAIL'		=> $data['shop_group_thumbnail'],
    'S_ENABLED'			=> ($data['shop_group_enabled'])? 'checked' : '',
    'S_ALLOW_ADS'		=> ($data['shop_group_allow_ads'])? 'checked' : '',
