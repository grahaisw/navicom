    ...
    if (window.focus) {
        newWindow.focus();
			...
			<tr>
			  ...
			  <!--<th><?php echo ((isset($this->_rootref['L_QTY'])) ? $this->_rootref['L_QTY'] : ((isset($user->lang['QTY'])) ? $user->lang['QTY'] : '{ QTY }')); ?></th>
			   -->
			  <th><?php echo ((isset($this->_rootref['L_DETAIL'])) ? $this->_rootref['L_DETAIL'] : ((isset($user->lang['DETAIL'])) ? $user->lang['DETAIL'] : '{ DETAIL }')); ?></th>
			...
			<tbody>
			<?php if ($this->_rootref['S_BUFFERS']) {  $_buffer_count = (isset($this->_tpldata['buffer'])) ? sizeof($this->_tpldata['buffer']) : 0;if ($_buffer_count) {for ($_buffer_i = 0; $_buffer_i < $_buffer_count; ++$_buffer_i){$_buffer_val = &$this->_tpldata['buffer'][$_buffer_i]; if (!($_buffer_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_buffer_val['DATETIME']; ?></td>
			    <td><?php echo $_buffer_val['ROOM']; ?></td>
			    <td><?php echo $_buffer_val['RESV_ID']; ?> - <?php echo $_buffer_val['NAME']; ?></td>
			    <td><?php echo $_buffer_val['ITEM']; ?></td>
			    <!--<td><?php echo $_buffer_val['QTY']; ?></td>
			    <td><?php echo $_buffer_val['NOTE']; ?></td> -->
			    <td><?php if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    
			    <a onclick="PopupCenter('<?php echo $_buffer_val['U_URL']; ?>', '<?php echo $_buffer_val['RESV_ID']; ?>',700,450);" href="javascript:void(0);"><img src="<?php echo $_buffer_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_buffer_val['L_UPDATE']; ?>" title="<?php echo $_buffer_val['L_UPDATE']; ?>" /></a>
