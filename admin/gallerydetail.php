...
$u_action = $tonjaw_admin_path . 'gallerydetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
	...
	'galleries_image_enabled'	=> (int) $image_enabled,
	'galleries_clip'		=> $clip,
	'galleries_clip_enabled'	=> (int) $clip_enabled,
	'galleries_order'		=> (int) ($order)? $order : '1',
	'galleries_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . GALLERIES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . GALLERIES_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE galleries_id = $did";
	...
	$galleries_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$galleries_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'galleries_id'		=> (int) $did,
	    'translation_title'		=> (string) $galleries_title,
	    'translation_description'	=> (string) $galleries_content,
	...
	{
	    $sql = 'INSERT INTO ' . GALLERY_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . GALLERY_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'gallery.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . GALLERIES_TABLE . " WHERE galleries_id = $did" ;
    ...
    $data_thumbnail = ($data['galleries_image'])? $data['galleries_image'] : '0';
    $data_clip = ($data['galleries_clip'])? $data['galleries_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['galleries_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . GALLERY_TRANSLATIONS_TABLE . " WHERE galleries_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}gallerydetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['galleries_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['galleries_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['galleries_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['galleries_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['galleries_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['galleries_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['galleries_order'],
	    'S_CLIP_ENABLED'	=> ($data['galleries_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['galleries_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['galleries_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}gallerydetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
