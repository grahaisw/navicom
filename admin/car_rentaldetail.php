...
$u_action = $tonjaw_admin_path . 'car_rentaldetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'car_rentals_image'		=> $image,
	'car_rentals_image_enabled'	=> (int) $image_enabled,
	'car_rentals_clip'		=> $clip,
	'car_rentals_clip_enabled'	=> (int) $clip_enabled,
	'car_rentals_order'		=> (int) ($order)? $order : '1',
	'car_rentals_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . CAR_RENTALS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . CAR_RENTALS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE car_rentals_id = $did";
	...
	$car_rentals_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$car_rentals_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'car_rentals_id'		=> (int) $did,
	    'translation_title'		=> (string) $car_rentals_title,
	    'translation_description'	=> (string) $car_rentals_content,
	...
	{
	    $sql = 'INSERT INTO ' . CAR_RENTAL_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . CAR_RENTAL_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'car_rental.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . CAR_RENTALS_TABLE . " WHERE car_rentals_id = $did" ;
    ...
    $data_thumbnail = ($data['car_rentals_image'])? $data['car_rentals_image'] : '0';
    $data_clip = ($data['car_rentals_clip'])? $data['car_rentals_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['car_rentals_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . CAR_RENTAL_TRANSLATIONS_TABLE . " WHERE car_rentals_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}car_rentaldetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['car_rentals_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['car_rentals_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['car_rentals_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['car_rentals_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['car_rentals_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['car_rentals_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['car_rentals_order'],
	    'S_CLIP_ENABLED'	=> ($data['car_rentals_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['car_rentals_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['car_rentals_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}car_rentaldetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
