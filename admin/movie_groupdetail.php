...
$u_action = $tonjaw_admin_path . 'movie_groupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'movie_group_description'	=> $description,
	'movie_group_enabled'		=> (int) $enabled_flag,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . MOVIE_GROUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . MOVIE_GROUPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE movie_group_id = $gid";
	...
	{
	    $sql = 'INSERT INTO ' . MOVIE_GROUP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . MOVIE_GROUP_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'movie_group.' . $phpEx, $sid);
}
...
if ($mode === 'update')
{
    if (empty($gid))
    {
	die('Missing Movie Category ID. Cannot update Movie Category Table.');
    }
    ...
    $sql = 'SELECT * FROM ' . MOVIE_GROUPS_TABLE . " WHERE movie_group_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . MOVIE_GROUP_TRANSLATIONS_TABLE . " WHERE movie_group_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}movie_groupdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
    ...
    'S_TITLE'			=> $data['movie_group_name'],
    'S_DESCRIPTION'		=> $data['movie_group_description'],
    'S_ENABLED'			=> ($data['movie_group_enabled'])? 'checked' : '',
...
$template->set_filenames(array(
	'body' => 'admin_movie_groupform.tpl',
));
