...
$u_action = $tonjaw_admin_path . 'laundrydetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'laundry_image'		=> $image,
	'laundry_image_enabled'	=> (int) $image_enabled,
	'laundry_clip'		=> $clip,
	'laundry_clip_enabled'	=> (int) $clip_enabled,
	'laundry_order'		=> (int) ($order)? $order : '1',
	'laundry_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . LAUNDRY_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . LAUNDRY_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE laundry_id = $did";
	...
	$laundry_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$laundry_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'laundry_id'		=> (int) $did,
	    'translation_title'		=> (string) $laundry_title,
	    'translation_description'	=> (string) $laundry_content,
	...
	{
	    $sql = 'INSERT INTO ' . LAUNDRY_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . LAUNDRY_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'laundry.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . LAUNDRY_TABLE . " WHERE laundry_id = $did" ;
    ...
    $data_thumbnail = ($data['laundry_image'])? $data['laundry_image'] : '0';
    $data_clip = ($data['laundry_clip'])? $data['laundry_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['laundry_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . LAUNDRY_TRANSLATIONS_TABLE . " WHERE laundry_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}laundrydetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['laundry_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['laundry_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['laundry_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['laundry_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['laundry_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['laundry_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['laundry_order'],
	    'S_CLIP_ENABLED'	=> ($data['laundry_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['laundry_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['laundry_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}laundrydetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
