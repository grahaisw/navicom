...
$u_action = append_sid("{$tonjaw_admin_path}business_center.$phpEx", "mode=update");
...
$start = grab_directory_grouping($directory_data, $directory_count, "business_center");
//print_r($start); exit;
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['business_centers_id'])) ? request_var('business_centers_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . BUSINESS_CENTERS_TABLE . ' 
	  SET business_centers_enabled=' . (string) $mark[$i] ."
	  WHERE business_centers_id = '" . $mid[$i] . "'";
...
foreach ($directory_data as $row)
{
	...
	'S_DID'			=> $row['id'],
	'TITLE'			=> $row['title'],
	'U_TITLE'		=> append_sid("{$tonjaw_admin_path}business_centerdetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}business_centerdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}business_center.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->set_filenames(array(
	'body' => 'admin_business_center.tpl',
));
