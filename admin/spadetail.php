...
$u_action = $tonjaw_admin_path . 'spadetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'spa_order'		=> (int) $order,
	'spa_enabled'		=> (int) $enabled_flag,
	'spa_allow_ads'		=> (int) $allow_ads_flag,
	'spa_price'		=> (int) $price,
	'spa_thumbnail'		=> (string) $thumbnail,
	'spa_group_id'		=> (int) $gid,
	'spa_code'		=> (string) $code,
	'spa_clip'		=> (string) $clip,
	'spa_currency'		=> (string) $currency,
     );
	...
	$sql = 'INSERT INTO ' . SPAS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	//echo $sql; exit;
	$db->sql_query($sql);
	$rid = $db->sql_nextid();
	...
	{
	    $sql = 'INSERT INTO ' . SPA_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . SPA_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'spa.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($rid))
    {
	die('Missing Spa ID. Cannot update Spa Table.');
    }
    
    $label = ($mode === 'update') ? $adm_lang['update_item'] : $adm_lang['view_item'];
    // Get spa data for updating
    $sql = 'SELECT s.*, t.translation_title AS group_name FROM ' . SPAS_TABLE . " s, " . 
	SPA_GROUPS_TABLE. " g, " . SPA_GROUP_TRANSLATIONS_TABLE . " t 
	WHERE s.spa_group_id=g.spa_group_id 
	AND g.spa_group_id=t.spa_group_id 
	AND t.language_id='" . $config['default_language'] . "'
	AND s.spa_id=" . (int) $rid;
    ...
    $data_thumbnail = ($data['spa_thumbnail'])? $data['spa_thumbnail'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['spa_icon_path'] . $data_thumbnail;
    
    $data_clip = ($data['spa_clip'])? $data['spa_clip'] : '0';
    $clip = $tonjaw_root_path . $config['media_path'] . $config['spa_clip_path'] . $data_clip;

    $sql = 'SELECT * FROM ' . SPA_TRANSLATIONS_TABLE . " WHERE spa_id = $rid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
if ( $pms_config['spa_item_in_pos'] )
{
    ...
    $pms	= new $pms_api();
    $pms->get_menu_item($data['spa_code']);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}spadetail.$phpEx", "mode=add") . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
    ...
    'S_THUMBNAIL'		=> $data['spa_thumbnail'],
    'S_CLIP_FILE'		=> $clip,
    'S_CLIP'			=> $data['spa_clip'],
    'S_ORDER'			=> $data['spa_order'],
    'S_PRICE'			=> $data['spa_price'],
    'S_POS_PRICE'		=> $price,
    'S_CODE'			=> $data['spa_code'],
    'L_LABEL'			=> $label,
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['spa_thumbnail'],
	    'S_CLIP'		=> $data['spa_clip'],
	    'S_GROUP'			=> generate_spa_groups('group_id', $data['spa_group_id']),
	    'S_CODE'			=> $data['spa_code'],
	    'S_PRICE'			=> $data['spa_price'],
	    'S_POS_PRICE'		=> $price,
	    'S_FORM'			=> '1',
	    'S_ENABLED'			=> ($data['spa_enabled'])? 'checked' : '',
	    'S_ALLOW_ADS'		=> ($data['spa_allow_ads'])? 'checked' : '',
	    'S_ORDER'			=> $data['spa_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'S_FORM_TOKEN'		=> $s_hidden_fields,
		'S_CURRENCY'		=> generate_currency('currency', $data['spa_currency']),
	));
	...
	$template->assign_vars(array(
	    ...
	    'S_CLIP'		=> $clip,
	    'S_DETAIL'		=> '1',
	    'S_ALLOW_ADS'	=> ($data['spa_allow_ads'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['spa_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ORDER'		=> $data['spa_order'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}spadetail.$phpEx", "mode=update") . '&amp;id=' .$rid . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
		'S_CURRENCY'	=> $data['spa_currency'],
	));
