...
if($keyword === 'all')
{
    $keyword = "WHERE zone_enabled = '1'";
}

$u_action = append_sid("{$tonjaw_admin_path}zone.$phpEx", "mode=update");

//GRAB ZONE DATA
$zone_data = array();
$zone_count = 0;
//$sql_sort = 'log_time DESC';
$start = view_zones($zone_data, $zone_count);
...
if ($mode === 'update')
{
    ...
    $zid	= (isset($_REQUEST['zid'])) ? request_var('zid', array(0)) : array();

    $i = 0;
    foreach($zone_data as $row)
    {

	$mark[$i] = request_var('mark_' . $zid[$i], '')? '1' : '0';
	
	$sql = 'UPDATE ' . ZONES_TABLE . ' 
	  SET zone_enabled = ' . (string) $mark[$i] ."
	  WHERE zone_id=" . $zid[$i];
...
if (isset($_GET['id']) && $mode === 'delete')
{
    ...
    $sql = 'DELETE FROM ' . ZONES_TABLE . ' WHERE zone_id = ' . (int) $zid;
    //$db->sql_query($sql);
...
foreach ($zone_data as $row)
{
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}zonedetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}zone.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}zonedetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_DATATABLE_NODES'		=> '1',
    'S_THIRD_FIELD'		=> '1',
    'L_ID'			=> $adm_lang['code'],
