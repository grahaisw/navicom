...
$u_action = $tonjaw_admin_path . 'guest_groupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    {
    $error = true;
    $error_msg = 'Guest Group Code field cannot be left empty.';
    ...
    $sql_ary = array(
        ...
        'guest_groups_enabled'       => (int) $enabled_flag,
        'guest_groups_code'      => (string) strtoupper($code),
        'guest_groups_name'      => (string) $name,
    );

    if ($mode === 'add')
    {
      
        $sql = 'INSERT INTO ' . GUEST_GROUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
        $sql = 'UPDATE ' . GUEST_GROUPS_TABLE . " SET " . 
        $db->sql_build_array('UPDATE', $sql_ary) .
        " WHERE guest_groups_id = $aid";
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    $sql = 'SELECT * FROM ' . GUEST_GROUPS_TABLE . " WHERE guest_groups_id=" . (int) $aid;
...
$template->assign_vars(array(
    ...
    'L_LABEL'           => $label,
    'S_CODE'            => $data['guest_groups_code'],
    'S_NAME'            => prepare_message($data['guest_groups_name']),
...
switch( $mode )
{
    ...
    $template->assign_vars(array(
        ...
        'V_ENABLED'     => ($data['guest_groups_enabled'])? 'checked' : '',
    ...
    $template->assign_vars(array(
        ...
        'S_ENABLED'     => ($data['guest_groups_enabled'])? 'True' : 'False',
        'L_EDIT'        => $adm_lang['update'],
        'U_EDIT'        => append_sid("{$tonjaw_admin_path}guest_groupdetail.$phpEx", "mode=update") . '&amp;id=' .$aid . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
    ));
...
$template->set_filenames(array(
    'body' => 'admin_guest_groupform.tpl',
));
