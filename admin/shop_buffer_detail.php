...
$sql = 'SELECT COUNT(d.guest_services_detail_id) AS total_entries 
    ...
    WHERE s.guest_service_id=d.guest_service_id AND s.guest_service_id=$bid AND s.guest_service_type='" . $config['shop_buffer_type'] . "'";
...
if ( $service_count > 0 )
{
	...
	" d WHERE s.guest_service_id=d.guest_service_id AND s.guest_service_id=$bid AND s.guest_service_type='" . $config['shop_buffer_type'] . "'";
	...
	$url_delete	= $tonjaw_admin_path . "shop_buffer_detail.$phpEx?mode=delete&id=$bid&did=" . $row['guest_services_detail_id'];
	...
	'U_APPROVE_URL'	=> $tonjaw_admin_path."shop_buffer.$phpEx?sid=$sid&mode=approve&id=$bid",
	'L_DECLINE'	=> $adm_lang['decline'],
	'U_DECLINE_URL'	=> $tonjaw_admin_path."shop_buffer.$phpEx?sid=$sid&mode=decline&id=$bid",
	'ICON_PATH'	=> $tonjaw_root_path . $config['imageset_path'],
