...
$u_action = $tonjaw_admin_path . 'publicplacedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'publicplaces_image'		=> $image,
	'publicplaces_image_enabled'	=> (int) $image_enabled,
	'publicplaces_clip'		=> $clip,
	'publicplaces_clip_enabled'	=> (int) $clip_enabled,
	'publicplaces_order'		=> (int) ($order)? $order : '1',
	'publicplaces_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . PUBLICPLACES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . PUBLICPLACES_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE publicplaces_id = $did";
	...
	$publicplaces_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$publicplaces_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'publicplaces_id'		=> (int) $did,
	    'translation_title'		=> (string) $publicplaces_title,
	    'translation_description'	=> (string) $publicplaces_content,
	...
	{
	    $sql = 'INSERT INTO ' . PUBLICPLACE_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . PUBLICPLACE_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'publicplace.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . PUBLICPLACES_TABLE . " WHERE publicplaces_id = $did" ;
    ...
    $data_thumbnail = ($data['publicplaces_image'])? $data['publicplaces_image'] : '0';
    $data_clip = ($data['publicplaces_clip'])? $data['publicplaces_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['publicplaces_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . PUBLICPLACE_TRANSLATIONS_TABLE . " WHERE publicplaces_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}publicplacedetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['publicplaces_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['publicplaces_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['publicplaces_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['publicplaces_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['publicplaces_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['publicplaces_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['publicplaces_order'],
	    'S_CLIP_ENABLED'	=> ($data['publicplaces_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['publicplaces_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['publicplaces_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}publicplacedetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_publicplaceform.tpl',
));
