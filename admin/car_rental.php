...
$u_action = append_sid("{$tonjaw_admin_path}car_rental.$phpEx", "mode=update");
...
$start = grab_directory_grouping($directory_data, $directory_count, "car_rental");
//print_r($start); exit;
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['car_rentals_id'])) ? request_var('car_rentals_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . CAR_RENTALS_TABLE . ' 
	  SET car_rentals_enabled=' . (string) $mark[$i] ."
	  WHERE car_rentals_id = '" . $mid[$i] . "'";
...
foreach ($directory_data as $row)
{
	...
	'S_DID'			=> $row['id'],
	'TITLE'			=> $row['title'],
	'U_TITLE'		=> append_sid("{$tonjaw_admin_path}car_rentaldetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}car_rentaldetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}car_rental.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
