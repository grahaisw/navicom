...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tbody>
			<?php if ($this->_rootref['S_ZONE']) {  $_zone_count = (isset($this->_tpldata['zone'])) ? sizeof($this->_tpldata['zone']) : 0;if ($_zone_count) {for ($_zone_i = 0; $_zone_i < $_zone_count; ++$_zone_i){$_zone_val = &$this->_tpldata['zone'][$_zone_i]; if (!($_zone_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_zone_val['NAME']; ?></td>
			    <td><?php echo $_zone_val['DESCRIPTION']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="hidden" name="zid[]" value="<?php echo $_zone_val['S_ZID']; ?>"/>
			    <input type="checkbox" name="mark_<?php echo $_zone_val['S_ZID']; ?>" <?php echo $_zone_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <td style="width: 5%" align="center"><a href="<?php echo $_zone_val['U_UPDATE']; ?>" rel="facebox"><img src="<?php echo $_zone_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_zone_val['L_UPDATE']; ?>" title="<?php echo $_zone_val['L_UPDATE']; ?>" /></a></td>
			    <?php } else { ?>

			    <td><?php echo $_zone_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_zone_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_zone_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_zone_val['L_DELETE']; ?>" title="<?php echo $_zone_val['L_DELETE']; ?>" /></a></td>
			    <?php } ?>
			...
			<?php }} } else { ?>
			  ...
			  <tr>
			    <td class="bg1" colspan="<?php if ($this->_rootref['S_ADD_UPDATE']) {  ?>4<?php } else { ?>2<?php } ?>" align="center"><span class="gen"><?php echo ((isset($this->_rootref['L_NO_ENTRIES'])) ? $this->_rootref['L_NO_ENTRIES'] : ((isset($user->lang['NO_ENTRIES'])) ? $user->lang['NO_ENTRIES'] : '{ NO_ENTRIES }')); ?></span></td>
			  </tr>
			<?php } ?>
