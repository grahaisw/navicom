...
$u_action = $tonjaw_admin_path . 'doctordetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'doctors_image'		=> $image,
	'doctors_image_enabled'	=> (int) $image_enabled,
	'doctors_clip'		=> $clip,
	'doctors_clip_enabled'	=> (int) $clip_enabled,
	'doctors_order'		=> (int) ($order)? $order : '1',
	'doctors_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . DOCTORS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	...
	$doctors_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$doctors_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'doctors_id'		=> (int) $did,
	    'translation_title'		=> (string) $doctors_title,
	    'translation_description'	=> (string) $doctors_content,
	...
	{
	    $sql = 'INSERT INTO ' . DOCTOR_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . DOCTOR_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'doctor.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . DOCTORS_TABLE . " WHERE doctors_id = $did" ;
    ...
    $data_thumbnail = ($data['doctors_image'])? $data['doctors_image'] : '0';
    $data_clip = ($data['doctors_clip'])? $data['doctors_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['doctors_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . DOCTOR_TRANSLATIONS_TABLE . " WHERE doctors_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}doctordetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['doctors_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['doctors_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['doctors_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['doctors_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['doctors_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['doctors_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['doctors_order'],
	    'S_CLIP_ENABLED'	=> ($data['doctors_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['doctors_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['doctors_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}doctordetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
