    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tr>
			  ...
			  <th><?php echo ((isset($this->_rootref['L_ACTIVE'])) ? $this->_rootref['L_ACTIVE'] : ((isset($user->lang['ACTIVE'])) ? $user->lang['ACTIVE'] : '{ ACTIVE }')); ?></th>
			  <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><th>&nbsp;</th><?php } if ($this->_rootref['S_DELETE']) {  ?><th>&nbsp;</th><?php } ?>

			</tr>
			</thead>
			<tbody>
			<?php if ($this->_rootref['S_STYLES']) {  $_style_count = (isset($this->_tpldata['style'])) ? sizeof($this->_tpldata['style']) : 0;if ($_style_count) {for ($_style_i = 0; $_style_i < $_style_count; ++$_style_i){$_style_val = &$this->_tpldata['style'][$_style_i]; if (!($_style_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_style_val['NAME']; ?></td>
			    <td><?php echo $_style_val['DESCRIPTION']; ?></td>
			    <td><?php echo $_style_val['TYPE']; ?></td>
			    <td><?php echo $_style_val['EXIST']; ?>

				<input type="hidden" name="style_id[]" value="<?php echo $_style_val['S_NID']; ?>"/>
			    </td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_style_val['S_NID']; ?>" <?php echo $_style_val['V_ACTIVE']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_style_val['ACTIVE']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_style_val['U_UPDATE']; ?>" rel="facebox"><img src="<?php echo $_style_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_style_val['L_UPDATE']; ?>" title="<?php echo $_style_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_style_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_style_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_style_val['L_DELETE']; ?>" title="<?php echo $_style_val['L_DELETE']; ?>" /></a></td>
			    <?php } ?>
