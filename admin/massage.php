...
$u_action = append_sid("{$tonjaw_admin_path}massage.$phpEx", "mode=update");
...
$start = grab_directory_grouping($directory_data, $directory_count, "massage");
//print_r($start); exit;
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['massage_id'])) ? request_var('massage_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . MASSAGES_TABLE . ' 
	  SET massage_enabled=' . (string) $mark[$i] ."
	  WHERE massage_id = '" . $mid[$i] . "'";
...
foreach ($directory_data as $row)
{
	...
	'S_DID'			=> $row['id'],
	'TITLE'			=> $row['title'],
	'U_TITLE'		=> append_sid("{$tonjaw_admin_path}massagedetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}massagedetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}massage.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
