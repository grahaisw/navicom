...
$u_action = $tonjaw_admin_path . 'music_groupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'music_group_description'	=> $description,
	'music_group_enabled'		=> (int) $enabled_flag,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . MUSIC_GROUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	
	// echo $sql . 'master<p>'; exit;
	$db->sql_query($sql);
	$gid = $db->sql_nextid();
    // echo $gid;exit();
	// $oke = $sql_ary['music_group_id'];
    }
    
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . MUSIC_GROUPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE music_group_id = $gid";
	...
	{
	    $sql = 'INSERT INTO ' . MUSIC_GROUP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . MUSIC_GROUP_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'music_group.' . $phpEx, $sid);
}
...
if ($mode === 'update')
{
    if (empty($gid))
    {
	die('Missing music Category ID. Cannot update music Category Table.');
    }
    ...
    $sql = 'SELECT * FROM ' . MUSIC_GROUPS_TABLE . " WHERE music_group_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . MUSIC_GROUP_TRANSLATIONS_TABLE . " WHERE music_group_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}music_groupdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
    ...
    'S_TITLE'			=> $data['music_group_name'],
    'S_DESCRIPTION'		=> $data['music_group_description'],
    'S_ENABLED'			=> ($data['music_group_enabled'])? 'checked' : '',
...
$template->set_filenames(array(
	'body' => 'admin_music_groupform.tpl',
));
