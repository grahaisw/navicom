...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tbody>
			<?php if ($this->_rootref['S_TERAPHISTS']) {  $_teraphist_count = (isset($this->_tpldata['teraphist'])) ? sizeof($this->_tpldata['teraphist']) : 0;if ($_teraphist_count) {for ($_teraphist_i = 0; $_teraphist_i < $_teraphist_count; ++$_teraphist_i){$_teraphist_val = &$this->_tpldata['teraphist'][$_teraphist_i]; if (!($_teraphist_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><img src="<?php echo $_teraphist_val['THUMBNAIL']; ?>" alt="<?php echo $_teraphist_val['NAME']; ?>" width="60"></td>
			    <td><strong><?php echo $_teraphist_val['NAME']; ?></strong><br/><?php echo $_teraphist_val['DESCRIPTION']; ?>

			    <input type="hidden" name="teraphist_id[]" value="<?php echo $_teraphist_val['S_GID']; ?>"/></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_teraphist_val['S_GID']; ?>" <?php echo $_teraphist_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_teraphist_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_teraphist_val['U_UPDATE']; ?>" rel="facebox"><img src="<?php echo $_teraphist_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_teraphist_val['L_UPDATE']; ?>" title="<?php echo $_teraphist_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_teraphist_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_teraphist_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_teraphist_val['L_DELETE']; ?>" title="<?php echo $_teraphist_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
