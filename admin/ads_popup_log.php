...
$u_action = append_sid("{$tonjaw_admin_path}ads_popup_log.$phpEx", "");
...
$start = view_ads_logs($log_data, $log_count, 'popup', $mode);
...
foreach ($log_data as $row)
{
	...
	'NAME'				=> $row['name'],
	'U_TIME'			=> append_sid("{$tonjaw_admin_path}ads_popup_logdetail.$phpEx", "mode=view") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
	...
	'S_TYPE'			=> strtolower($adm_lang['popup']),
	'S_DATEFROM'		=> date("Y-m-d"),
	'S_DATEEND'			=> date("Y-m-d"),
    ));

$template->set_filenames(array(
	'body' => 'admin_ads_popup_log.tpl',
));
