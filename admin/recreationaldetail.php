...
$u_action = $tonjaw_admin_path . 'recreationaldetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'recreationals_image'		=> $image,
	'recreationals_image_enabled'	=> (int) $image_enabled,
	'recreationals_clip'		=> $clip,
	'recreationals_clip_enabled'	=> (int) $clip_enabled,
	'recreationals_order'		=> (int) ($order)? $order : '1',
	'recreationals_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . RECREATIONALS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . RECREATIONALS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE recreationals_id = $did";
	...
	$recreationals_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$recreationals_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'recreationals_id'		=> (int) $did,
	    'translation_title'		=> (string) $recreationals_title,
	    'translation_description'	=> (string) $recreationals_content,
	...
	{
	    $sql = 'INSERT INTO ' . RECREATIONAL_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . RECREATIONAL_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'recreational.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . RECREATIONALS_TABLE . " WHERE recreationals_id = $did" ;
    ...
    $data_thumbnail = ($data['recreationals_image'])? $data['recreationals_image'] : '0';
    $data_clip = ($data['recreationals_clip'])? $data['recreationals_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['recreationals_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . RECREATIONAL_TRANSLATIONS_TABLE . " WHERE recreationals_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}recreationaldetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['recreationals_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['recreationals_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['recreationals_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['recreationals_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['recreationals_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['recreationals_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['recreationals_order'],
	    'S_CLIP_ENABLED'	=> ($data['recreationals_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['recreationals_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['recreationals_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}recreationaldetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_recreationalform.tpl',
));
