...
$u_action = append_sid("{$tonjaw_admin_path}music_background.$phpEx", "mode=update");
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['music_id'])) ? request_var('music_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . BACKGROUND_MUSIC_TABLE . ' 
	  SET background_music_enabled=' . (string) $mark[$i] ."
	  WHERE background_music_id = '" . $mid[$i] . "'";
...
if (isset($_GET['id']) && $mode === 'delete')
{
    ...
    // Delete files in temporary folder
        ...
        // $files = glob($tonjaw_admin_path . '../../vod/music_background/'.$music.''); //get all file names
    ...
    $sql = 'DELETE FROM ' . BACKGROUND_MUSIC_TABLE . ' WHERE background_music_id = ' . (int) $mid;
    $db->sql_query($sql);
    $sql2 = 'DELETE FROM ' . BACKGROUND_MUSIC_GROUPINGS_TABLE . ' WHERE background_music_id = ' . (int) $mid;
    $db->sql_query($sql2);
...
{ 
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}music_backgrounddetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_NAME'		=> append_sid("{$tonjaw_admin_path}music_background.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}music_background.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}music_backgrounddetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '1',
    'S_FOURTH_FIELD'		=> '1',
...
$template->set_filenames(array(
	'body' => 'admin_musicbackground.tpl',
));
