...
/**
*
* admin/massagedetail.php
*
* Agnes Emanuella. Sep 2015
*/
...
$u_action = $tonjaw_admin_path . 'massagedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'massage_image'		=> $image,
	'massage_image_enabled'	=> (int) $image_enabled,
	'massage_clip'		=> $clip,
	'massage_clip_enabled'	=> (int) $clip_enabled,
	'massage_order'		=> (int) ($order)? $order : '1',
	'massage_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . MASSAGES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . MASSAGES_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE massage_id = $did";
	...
	$massage_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$massage_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'massage_id'		=> (int) $did,
	    'translation_title'		=> (string) $massage_title,
	    'translation_description'	=> (string) $massage_content,
	...
	{
	    $sql = 'INSERT INTO ' . MASSAGE_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . MASSAGE_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'massage.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . MASSAGES_TABLE . " WHERE massage_id = $did" ;
    ...
    $data_thumbnail = ($data['massage_image'])? $data['massage_image'] : '0';
    $data_clip = ($data['massage_clip'])? $data['massage_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['massage_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . MASSAGE_TRANSLATIONS_TABLE . " WHERE massage_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}massagedetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['massage_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['massage_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['massage_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['massage_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['massage_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['massage_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['massage_order'],
	    'S_CLIP_ENABLED'	=> ($data['massage_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['massage_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['massage_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}massagedetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
