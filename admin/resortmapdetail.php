...
$u_action = $tonjaw_admin_path . 'resortmapdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'resortmaps_image'		=> $image,
	'resortmaps_image_enabled'	=> (int) $image_enabled,
	'resortmaps_clip'		=> $clip,
	'resortmaps_clip_enabled'	=> (int) $clip_enabled,
	'resortmaps_order'		=> (int) ($order)? $order : '1',
	'resortmaps_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . RESORTMAPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . RESORTMAPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE resortmaps_id = $did";
	...
	{
	    $sql = 'INSERT INTO ' . RESORTMAP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . RESORTMAP_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'resortmap.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . RESORTMAPS_TABLE . " WHERE resortmaps_id = $did" ;
    ...
    $data_thumbnail = ($data['resortmaps_image'])? $data['resortmaps_image'] : '0';
    $data_clip = ($data['resortmaps_clip'])? $data['resortmaps_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['resortmaps_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . RESORTMAP_TRANSLATIONS_TABLE . " WHERE resortmaps_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}resortmapdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['resortmaps_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['resortmaps_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['resortmaps_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['resortmaps_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['resortmaps_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['resortmaps_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['resortmaps_order'],
	    'S_CLIP_ENABLED'	=> ($data['resortmaps_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['resortmaps_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['resortmaps_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}resortmapdetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
