...
$u_action = append_sid("{$tonjaw_admin_path}service.$phpEx", "mode=update");
...
$start = grab_services($service_data, $service_count);
...
if ($mode === 'update')
{
    ...
    $rid	= (isset($_REQUEST['service_id'])) ? request_var('service_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . SERVICES_TABLE . ' 
	  SET service_enabled=' . (string) $mark[$i] ."
	  WHERE service_id = '" . $rid[$i] . "'";
...
{
	$service_id = request_var('id', '');
    
    $sql = 'DELETE FROM ' . SERVICES_TABLE . ' WHERE service_id = ' . (int) $service_id;
    $db->sql_query($sql);
    
    redirect($config['admin_path'] . 'service.' . $phpEx, $sid);
}

$thumbnail_path = $tonjaw_root_path . $config['media_path'] . $config['service_icon_path'];
...
{
    //$data = array();
    $thumbnail = file_exists($thumbnail_path.$row['thumbnail'])? $thumbnail_path.$row['thumbnail'] : $thumbnail_path.$config['default_thumbnail_service'];
	...
	*/
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}servicedetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_NAME'		=> append_sid("{$tonjaw_admin_path}servicedetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}service.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}servicedetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '1',
    'S_THIRD_FIELD'		=> '1',
