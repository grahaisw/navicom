...
$u_action = $tonjaw_admin_path . 'tourdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'tour_order'		=> (int) $order,
	'tour_enabled'		=> (int) $enabled_flag,
	'tour_allow_ads'	=> (int) $allow_ads_flag,
	'tour_price'		=> (int) $price,
	'tour_thumbnail'	=> (string) $thumbnail,
	'tour_group_id'		=> (int) $gid,
	'tour_code'		=> (string) $code,
	'tour_clip'		=> (string) $clip,
	'tour_currency'		=> (string) $currency,
     );
	...
	$sql = 'INSERT INTO ' . TOURS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	//echo $sql; exit;
	$db->sql_query($sql);
	$rid = $db->sql_nextid();
	...
	{
	    $sql = 'INSERT INTO ' . TOUR_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . TOUR_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'tour.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($rid))
    {
	die('Missing Tour ID. Cannot update Tour Table.');
    }
    
    $label = ($mode === 'update') ? $adm_lang['update_item'] : $adm_lang['view_item'];
    // Get service data for updating
    $sql = 'SELECT s.*, t.translation_title AS group_name FROM ' . TOURS_TABLE . " s, " . 
	TOUR_GROUPS_TABLE. " g, " . TOUR_GROUP_TRANSLATIONS_TABLE . " t 
	WHERE s.tour_group_id=g.tour_group_id 
	AND g.tour_group_id=t.tour_group_id 
	AND t.language_id='" . $config['default_language'] . "'
	AND s.tour_id=" . (int) $rid;
    ...
    $data_thumbnail = ($data['tour_thumbnail'])? $data['tour_thumbnail'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['tour_icon_path'] . $data_thumbnail;
    
    $data_clip = ($data['tour_clip'])? $data['tour_clip'] : '0';
    $clip = $tonjaw_root_path . $config['media_path'] . $config['tour_clip_path'] . $data_thumbnail;

    $sql = 'SELECT * FROM ' . TOUR_TRANSLATIONS_TABLE . " WHERE tour_id = $rid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
if ( $pms_config['tour_item_in_pos'] )
{
    ...
    $pms	= new $pms_api();
    $pms->get_menu_item($data['tour_code']);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}tourdetail.$phpEx", "mode=add") . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
    ...
    'S_THUMBNAIL'		=> $data['tour_thumbnail'],
    'L_CLIP'			=> $adm_lang['clip'],
    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
    'S_CLIP_FILE'		=> $clip,
    'S_CLIP'			=> $data['tour_clip'],
    'S_ORDER'			=> $data['tour_order'],
    'S_PRICE'			=> $data['tour_price'],
    'S_POS_PRICE'		=> $price,
    'S_CODE'			=> $data['tour_code'],
    'L_LABEL'			=> $label,
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['tour_thumbnail'],
	    'S_CLIP'			=> $data['tour_clip'],
	    'S_GROUP'			=> generate_tour_groups('group_id', $data['tour_group_id']),
	    'S_CODE'			=> $data['tour_code'],
	    'S_PRICE'			=> $data['tour_price'],
	    'S_POS_PRICE'		=> $price,
	    'S_FORM'			=> '1',
	    'S_ENABLED'			=> ($data['tour_enabled'])? 'checked' : '',
	    'S_ALLOW_ADS'		=> ($data['tour_allow_ads'])? 'checked' : '',
	    'S_ORDER'			=> $data['tour_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'S_FORM_TOKEN'		=> $s_hidden_fields,
		'S_CURRENCY'		=> generate_currency('currency', $data['tour_currency']),
	));
	...
	$template->assign_vars(array(
	    ...
	    'S_CLIP'		=> $clip,
	    'S_DETAIL'		=> '1',
	    'S_ALLOW_ADS'	=> ($data['tour_allow_ads'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['tour_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ORDER'		=> $data['tour_order'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}tourdetail.$phpEx", "mode=update") . '&amp;id=' .$rid . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
		'S_CURRENCY'	=> $data['tour_currency'],
	));
