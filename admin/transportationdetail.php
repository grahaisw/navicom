...
/**
*
* admin/transportationdetail.php
*
...
$u_action = $tonjaw_admin_path . 'transportationdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
	...
	'car_enabled'	=> (int) $enabled_flag,
	'car_thumbnail'	=> $thumbnail,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . CARS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	...
	{
	    $sql = 'INSERT INTO ' . CAR_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . CAR_TRANSLATIONS_TABLE . " SET " . 
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($gid))
    {
	die('Missing Transportation ID. Cannot update Transportation Table.');
    }
    ...
    $sql = 'SELECT * FROM ' . CARS_TABLE . " WHERE car_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . CAR_TRANSLATIONS_TABLE . " WHERE car_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'S_THUMBNAIL'		=> $data['thumbnail'],
    'S_ENABLED'			=> ($data['enabled'])? 'checked' : '',
...
$template->set_filenames(array(
	'body' => 'admin_transportationform.tpl',
));
