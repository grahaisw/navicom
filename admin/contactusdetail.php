...
$u_action = $tonjaw_admin_path . 'contactusdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
	...
	'contactus_image_enabled'	=> (int) $image_enabled,
	'contactus_clip'		=> $clip,
	'contactus_clip_enabled'	=> (int) $clip_enabled,
	'contactus_order'		=> (int) ($order)? $order : '1',
	'contactus_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . CONTACTUS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	...
	$contactus_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$contactus_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'contactus_id'		=> (int) $did,
	    'translation_title'		=> (string) $contactus_title,
	    'translation_description'	=> (string) $contactus_content,
	...
	{
	    $sql = 'INSERT INTO ' . CONTACTUS_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . CONTACTUS_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'contactus.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . CONTACTUS_TABLE . " WHERE contactus_id = $did" ;
    ...
    $data_thumbnail = ($data['dinings_image'])? $data['dinings_image'] : '0';
    $data_clip = ($data['dinings_clip'])? $data['dinings_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['dinings_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . CONTACTUS_TRANSLATIONS_TABLE . " WHERE contactus_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}contactusdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['contactus_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['contactus_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['contactus_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['contactus_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['contactus_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['contactus_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_CLIP_FILE'	=> $clip,
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['dinings_order'],
	    'S_CLIP_ENABLED'	=> ($data['contactus_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['contactus_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['contactus_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}contactusdetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
