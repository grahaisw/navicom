...
$u_action = append_sid("{$tonjaw_admin_path}shopgroup.$phpEx", "mode=update");

//GRAB SHOP GROUP DATA
$group_data = array();
$group_count = 0;
//$sql_sort = 'log_time DESC';
$start = view_shop_groups($group_data, $group_count);
...
if ($mode === 'update')
{
	...
	$sql = 'UPDATE ' . SHOP_GROUPS_TABLE . ' 
	  SET shop_group_enabled=' . (string) $mark[$i] ."
	  WHERE shop_group_id = '" . $gid[$i] . "'";
...
$thumbnail_path = $tonjaw_root_path . $config['media_path'] . $config['shop_group_icon_path'];
...
foreach ($group_data as $row)
{
	...
	'ORDER'			=> $row['order'],
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}shopgroupdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}shopgroup.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
