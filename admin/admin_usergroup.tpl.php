...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tbody>
			<?php if ($this->_rootref['S_GROUP']) {  $_group_count = (isset($this->_tpldata['group'])) ? sizeof($this->_tpldata['group']) : 0;if ($_group_count) {for ($_group_i = 0; $_group_i < $_group_count; ++$_group_i){$_group_val = &$this->_tpldata['group'][$_group_i]; if (!($_group_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_group_val['NAME']; ?></td>
			    <td><?php echo $_group_val['DESCRIPTION']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="hidden" name="gid[]" value="<?php echo $_group_val['S_GID']; ?>"/>
			    ...
			    <td style="width: 5%" align="center"><a href="<?php echo $_group_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_group_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_group_val['L_DELETE']; ?>" title="<?php echo $_group_val['L_DELETE']; ?>" /></a></td>
			    <?php } ?>
