...
$u_action = $tonjaw_admin_path . 'spagroupdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
	...
	'spa_group_enabled'	=> (int) $enabled_flag,
	'spa_group_order'	=> (int) $order,
	'spa_group_allow_ads'	=> (int) $allow_ads,
	'spa_group_thumbnail'	=> $thumbnail,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . SPA_GROUPS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . SPA_GROUPS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE spa_group_id = $gid";
	...
	{
	    $sql = 'INSERT INTO ' . SPA_GROUP_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . SPA_GROUP_TRANSLATIONS_TABLE . " SET " . 
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($gid))
    {
	die('Missing Spa Group ID. Cannot update Spa Group Table.');
    }
    ...
    $sql = 'SELECT * FROM ' . SPA_GROUPS_TABLE . " WHERE spa_group_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . SPA_GROUP_TRANSLATIONS_TABLE . " WHERE spa_group_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'S_ORDER'			=> $data['spa_group_order'],
    'S_THUMBNAIL'		=> $data['spa_group_thumbnail'],
    'S_ENABLED'			=> ($data['spa_group_enabled'])? 'checked' : '',
    'S_ALLOW_ADS'		=> ($data['spa_group_allow_ads'])? 'checked' : '',
