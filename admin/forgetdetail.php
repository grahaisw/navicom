...
$u_action = $tonjaw_admin_path . 'forgetdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'forgets_image'		=> $image,
	'forgets_image_enabled'	=> (int) $image_enabled,
	'forgets_clip'		=> $clip,
	'forgets_clip_enabled'	=> (int) $clip_enabled,
	'forgets_order'		=> (int) ($order)? $order : '1',
	'forgets_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . FORGETS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . FORGETS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE forgets_id = $did";
	...
	$forgets_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$forgets_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'forgets_id'		=> (int) $did,
	    'translation_title'		=> (string) $forgets_title,
	    'translation_description'	=> (string) $forgets_content,
	...
	{
	    $sql = 'INSERT INTO ' . FORGET_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . FORGET_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'forget.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . FORGETS_TABLE . " WHERE forgets_id = $did" ;
    ...
    $data_thumbnail = ($data['forgets_image'])? $data['forgets_image'] : '0';
    $data_clip = ($data['forgets_clip'])? $data['forgets_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['forgets_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . FORGET_TRANSLATIONS_TABLE . " WHERE forgets_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}forgetdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['forgets_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['forgets_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['forgets_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['forgets_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['forgets_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['forgets_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['forgets_order'],
	    'S_CLIP_ENABLED'	=> ($data['forgets_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['forgets_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['forgets_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}forgetdetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
