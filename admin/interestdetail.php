...
/**
*
* admin/interestdetail.php
*
* Agnes Emanuella. Jul 2015
*/
...
$u_action = $tonjaw_admin_path . 'interestdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'interest_image'		=> $image,
	'interest_image_enabled'	=> (int) $image_enabled,
	'interest_clip'		=> $clip,
	'interest_clip_enabled'	=> (int) $clip_enabled,
	'interest_order'		=> (int) ($order)? $order : '1',
	'interest_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . INTERESTS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . INTERESTS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE interest_id = $did";
	...
	$interest_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$interest_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'interest_id'		=> (int) $did,
	    'translation_title'		=> (string) $interest_title,
	    'translation_description'	=> (string) $interest_content,
	...
	{
	    $sql = 'INSERT INTO ' . INTEREST_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . INTEREST_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'interest.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . INTERESTS_TABLE . " WHERE interest_id = $did" ;
    ...
    $data_thumbnail = ($data['interest_image'])? $data['interest_image'] : '0';
    $data_clip = ($data['interest_clip'])? $data['interest_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['interest_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . INTEREST_TRANSLATIONS_TABLE . " WHERE interest_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}interestdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['interest_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['interest_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['interest_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['interest_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['interest_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['interest_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['interest_order'],
	    'S_CLIP_ENABLED'	=> ($data['interest_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['interest_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['interest_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}interestdetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
