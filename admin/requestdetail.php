...
$u_action = $tonjaw_admin_path . 'requestdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $value = utf8_normalize_nfc(request_var('value', ''));
    $enabled_flag = request_var('enabled_flag', '');
    $enabled_flag = $enabled_flag == 'on' ? '1' : '0';
    
    $sql_ary = array(
	//'service_group_description'	=> $description,
	'guest_request_enabled'	=> (int) $enabled_flag,
	'guest_request_value'	=> $value,
    );

    if ($mode === 'add')
    {
      
	$sql = 'INSERT INTO ' . GUEST_REQUESTS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . GUEST_REQUESTS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE guest_request_id = $gid";
	...
	{
	    $sql = 'INSERT INTO ' . GUEST_REQUEST_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . GUEST_REQUEST_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'request.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($gid))
    {
	die('Missing Request ID. Cannot update Request Table.');
    }
    ...
    $sql = 'SELECT * FROM ' . GUEST_REQUESTS_TABLE . " WHERE guest_request_id=" . (int) $gid;
    ...
    $sql = 'SELECT * FROM ' . GUEST_REQUEST_TRANSLATIONS_TABLE . " WHERE guest_request_id = $gid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}requestdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
    'L_TITLE'			=> $adm_lang['name'],
    'L_VALUE'			=> $adm_lang['value'],
    'L_ENABLED'			=> $adm_lang['enabled'],
    'S_VALUE'			=> $data['guest_request_value'],
    'S_ENABLED'			=> ($data['guest_request_enabled'])? 'checked' : '',
