...
$u_action = $tonjaw_admin_path . 'meetingeventdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'meetingevents_image'		=> $image,
	'meetingevents_image_enabled'	=> (int) $image_enabled,
	'meetingevents_clip'		=> $clip,
	'meetingevents_clip_enabled'	=> (int) $clip_enabled,
	'meetingevents_order'		=> (int) ($order)? $order : '1',
	'meetingevents_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . MEETINGEVENTS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . MEETINGEVENTS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE meetingevents_id = $did";
	...
	$meetingevents_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$meetingevents_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'meetingevents_id'		=> (int) $did,
	    'translation_title'		=> (string) $meetingevents_title,
	    'translation_description'	=> (string) $meetingevents_content,
	...
	{
	    $sql = 'INSERT INTO ' . MEETINGEVENT_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . MEETINGEVENT_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'meetingevent.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . MEETINGEVENTS_TABLE . " WHERE meetingevents_id = $did" ;
    ...
    $data_thumbnail = ($data['meetingevents_image'])? $data['meetingevents_image'] : '0';
    $data_clip = ($data['meetingevents_clip'])? $data['meetingevents_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['meetingevents_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . MEETINGEVENT_TRANSLATIONS_TABLE . " WHERE meetingevents_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}meetingeventdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['meetingevents_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['meetingevents_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['meetingevents_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['meetingevents_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['meetingevents_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['meetingevents_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['meetingevents_order'],
	    'S_CLIP_ENABLED'	=> ($data['meetingevents_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['meetingevents_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['meetingevents_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}meetingeventdetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_meetingeventform.tpl',
));
