...
$u_action = $tonjaw_admin_path . 'moviedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $filename = '../../vod/movies/' . $url;
	$filename_music = '../vod/' . $url;
	
	if(file_exists($filename)) {
	 
		$uploadFile = '../../vod/movies/' . basename($url);
		$handle    = fopen('../../vod/movies/' . $url, "r");
    ...
    $sql_ary = array(
	    ...
	    'movie_url'		=> $url,
	    'movie_enabled'	=> (int) $enabled_flag,
	    'movie_allow_ads'	=> (int) $allow_ads_flag,
	    'movie_price'	=> (int) $price,
	    'movie_thumbnail'	=> $picture_name,
	    'movie_code'	=> (string) $code,
     );
    /*
	...
	*/
	...
	$sql = 'INSERT INTO ' . MOVIES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	//echo $sql; exit;
	$db->sql_query($sql);
	$mid = $db->sql_nextid();
    ...
    $music = $db->sql_fetchfield('movie_url');
    // echo $music;exit();
    // Delete files in temporary folder
      		$c = curl_init();
          	curl_setopt($c, CURLOPT_URL, $config['vod_server'].$config['vod_path']."/handle.php?mod=delete&type=movies&file=".$music);
                curl_setopt($c, CURLOPT_POST, 1);
                curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($c, CURLOPT_POSTFIELDS, $POST_DATA);
	...
	$sql = 'UPDATE ' . MOVIES_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE movie_id = $mid";
	$db->sql_query($sql);
	
	// Remove old data from movie grouping table
	$sql = 'DELETE FROM ' . MOVIE_GROUPINGS_TABLE . "
	    WHERE movie_id = $mid";
	$db->sql_query($sql);
	
    }
    //print_r($gid);exit;
    //Insert new data to movie_grouping table
	...
	$sql = 'INSERT INTO ' . MOVIE_GROUPINGS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	...
	{
	    $sql = 'INSERT INTO ' . MOVIE_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . MOVIE_TRANSLATIONS_TABLE . " SET " . 
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($mid))
    {
	die('Missing Movie ID. Cannot update Movie Table.');
    }
    
    $label = ($mode === 'update') ? $adm_lang['update_item'] : $adm_lang['view_item'];
    // Get movie data for updating
    $sql = 'SELECT * FROM ' . MOVIES_TABLE . " WHERE movie_id=" . (int) $mid;
    ...
    $data_thumbnail = ($data['movie_thumbnail'])? $data['movie_thumbnail'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['movie_icon_path'] . $data_thumbnail;
    $data_trailer = ($data['movie_trailer'])? $data['movie_trailer'] : '0';
    $trailer =  $tonjaw_root_path . $config['media_path'] . $config['trailer_path'] . $data_trailer;
    
    $sql = 'SELECT g.movie_group_id, t.translation_title FROM ' . MOVIE_GROUPINGS_TABLE . " g, " . 
	MOVIE_GROUP_TRANSLATIONS_TABLE . " t WHERE g.movie_id = $mid AND g.movie_group_id=t.movie_group_id 
    ...
    {
	$group_data[$i] = array(
	    'movie_group_id'	=> $detail['movie_group_id'],
    ...
    $sql = 'SELECT * FROM ' . MOVIE_TRANSLATIONS_TABLE . " WHERE movie_id = $mid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
    //$detail = $db->sql_fetchrow($result);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}moviedetail.$phpEx", "mode=add") . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
    'L_TITLE'			=> $adm_lang['title'],
    ...
    'S_THUMBNAIL'		=> $data['movie_thumbnail'],
    'S_DIRECTOR'		=> $data['movie_director'],
    'S_CASTS'			=> $data['movie_casts'],
    'S_URL'			=> $data['movie_url'],
    'S_PRICE'			=> $data['movie_price'],
    'S_CODE'			=> $data['movie_code'],
    'L_LABEL'			=> $label,
));
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['movie_thumbnail'],
	    'S_GENRE'			=> generate_movie_genre('genre_id[]', $mid, $group_data),
	    
	    'S_FORM'			=> '1',
	    'S_ENABLED'			=> ($data['movie_enabled'])? 'checked' : '',
	    'S_ALLOW_ADS'		=> ($data['movie_allow_ads'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_POSTER'		=> $thumbnail,
	    'S_DETAIL'		=> '1',
	    'S_ALLOW_ADS'	=> ($data['movie_allow_ads'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['movie_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_TRAILER'		=> $trailer,
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}moviedetail.$phpEx", "mode=update") . '&amp;id=' .$mid . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
