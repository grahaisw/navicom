...
$u_action = append_sid("{$tonjaw_admin_path}music.$phpEx", "mode=update");
...
$start = grab_music($music_data, $music_count);
...
if ($mode === 'update')
{
	...
	$sql = 'UPDATE ' . MUSIC_TABLE . ' 
	  SET music_enabled=' . (string) $mark[$i] ."
	  WHERE music_id = '" . $mid[$i] . "'";
...
if (isset($_GET['id']) && $mode === 'delete')
{
    ...
    $sql = 'DELETE FROM ' . MUSIC_TABLE . ' WHERE music_id = ' . (int) $mid;
    $db->sql_query($sql);
    $sql2 = 'DELETE FROM ' . MUSIC_GROUPINGS_TABLE . ' WHERE music_id = ' . (int) $mid;
    $db->sql_query($sql2);
    $sql3 = 'DELETE FROM ' . MUSIC_TRANSLATIONS_TABLE . ' WHERE music_id = ' . (int) $mid;
    $db->sql_query($sql3);
    // echo "string";exit();
    // print_r($sql);exit();
    redirect($config['admin_path'] . 'music.' . $phpEx, $sid);
...
{ 
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}musicdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_NAME'		=> append_sid("{$tonjaw_admin_path}musicdetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}music.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}musicdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '1',
    'S_FOURTH_FIELD'		=> '1',
