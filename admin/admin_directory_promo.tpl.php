...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tbody>
			<?php if ($this->_rootref['S_DIRECTORY']) {  $_directory_promo_count = (isset($this->_tpldata['directory_promo'])) ? sizeof($this->_tpldata['directory_promo']) : 0;if ($_directory_promo_count) {for ($_directory_promo_i = 0; $_directory_promo_i < $_directory_promo_count; ++$_directory_promo_i){$_directory_promo_val = &$this->_tpldata['directory_promo'][$_directory_promo_i]; if (!($_directory_promo_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_directory_promo_val['ORDER']; ?></td>
			    <td><a href="<?php echo $_directory_promo_val['U_TITLE']; ?>"><?php echo $_directory_promo_val['TITLE']; ?></a></td>
			    <td><img src="{directory_promo.THUMBNAIL" alt="<?php echo $_directory_promo_val['TITLE']; ?>" />
			    <input type="hidden" name="directory_id[]" value="<?php echo $_directory_promo_val['S_DID']; ?>"/></td>
			    <td><?php echo $_directory_promo_val['IMAGE_ENABLED']; ?></td>
			    <td><?php echo $_directory_promo_val['CLIP_ENABLED']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_directory_promo_val['S_DID']; ?>" <?php echo $_directory_promo_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_directory_promo_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_directory_promo_val['U_UPDATE']; ?>"><img src="<?php echo $_directory_promo_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_directory_promo_val['L_UPDATE']; ?>" title="<?php echo $_directory_promo_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_directory_promo_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_directory_promo_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_directory_promo_val['L_DELETE']; ?>" title="<?php echo $_directory_promo_val['L_DELETE']; ?>" /></a></td>
			    <?php } ?>
