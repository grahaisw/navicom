...
$start = grab_tv($tv_data, $tv_count);
...
{
	$tv_id = request_var('id', '');
    
    $sql = 'DELETE FROM ' . TV_CHANNELS_TABLE . ' WHERE tv_channel_id = ' . (int) $tv_id;
    $db->sql_query($sql);
...
foreach ($tv_data as $row)
{
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}tvdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}tv.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}tvdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '1',
    'S_FOURTH_FIELD'		=> '1',
