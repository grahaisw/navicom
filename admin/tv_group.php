...
$u_action = append_sid("{$tonjaw_admin_path}tv_group.$phpEx", "mode=update");

//GRAB TV GROUP DATA
$group_data = array();
$group_count = 0;
//$sql_sort = 'log_time DESC';
$start = view_tv_groups($group_data, $group_count);
...
if ($mode === 'update')
{
	...
	$sql = 'UPDATE ' . TV_GROUPS_TABLE . ' 
	  SET tv_channel_group_enabled=' . (string) $mark[$i] ."
	  WHERE tv_channel_group_id = '" . $gid[$i] . "'";
...
foreach ($group_data as $row)
{
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}tv_groupdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}tv_group.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
