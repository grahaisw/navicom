...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tbody>
			<?php if ($this->_rootref['S_MCAT']) {  $_mcat_count = (isset($this->_tpldata['mcat'])) ? sizeof($this->_tpldata['mcat']) : 0;if ($_mcat_count) {for ($_mcat_i = 0; $_mcat_i < $_mcat_count; ++$_mcat_i){$_mcat_val = &$this->_tpldata['mcat'][$_mcat_i]; if (!($_mcat_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_mcat_val['NAME']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_mcat_val['U_UPDATE']; ?>" rel="facebox"><img src="<?php echo $_mcat_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_mcat_val['L_UPDATE']; ?>" title="<?php echo $_mcat_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_mcat_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_mcat_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_mcat_val['L_DELETE']; ?>" title="<?php echo $_mcat_val['L_DELETE']; ?>" /></a></td>
			    <?php } ?>
