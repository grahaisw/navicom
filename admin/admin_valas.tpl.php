    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<?php if ($this->_rootref['S_VALAS']) {  $_valas_count = (isset($this->_tpldata['valas'])) ? sizeof($this->_tpldata['valas']) : 0;if ($_valas_count) {for ($_valas_i = 0; $_valas_i < $_valas_count; ++$_valas_i){$_valas_val = &$this->_tpldata['valas'][$_valas_i]; ?>
			    ...
			    <td><a href="<?php echo $_valas_val['U_NAME']; ?>"><?php echo $_valas_val['NAME']; ?></a></td>
			    <td><?php echo $_valas_val['BELI']; ?> </td>
			    <td><?php echo $_valas_val['JUAL']; ?><input type="hidden" name="valas_id[]" value="<?php echo $_valas_val['V_TID']; ?>"/></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_valas_val['U_UPDATE']; ?>"><img src="<?php echo $_valas_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_valas_val['L_UPDATE']; ?>" title="<?php echo $_valas_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_valas_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_valas_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_valas_val['L_DELETE']; ?>" title="<?php echo $_valas_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
