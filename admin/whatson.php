...
$u_action = append_sid("{$tonjaw_admin_path}whatson.$phpEx", "mode=update");

//GRAB DIRECTORY DATA
$directory_data = array();
$directory_count = 0;
//$sql_sort = 'log_time DESC';
$start = grab_directory_grouping($directory_data, $directory_count, "whatson");
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['whatson_id'])) ? request_var('whatson_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . WHATSON_TABLE . ' 
	  SET whatson_enabled=' . (string) $mark[$i] ."
	  WHERE whatson_id = '" . $mid[$i] . "'";
...
if (isset($_GET['id']) && $mode === 'delete')
{
    ...
    $sql = 'DELETE FROM ' . WHATSON_TABLE . ' WHERE whatson_id = ' . (int) $directory_id;
    $db->sql_query($sql);
    
    redirect($config['admin_path'] . 'whatson.' . $phpEx, $sid);
}

$thumbnail_path = $tonjaw_root_path . $config['media_path'] . $config['whatson_image_path'];
...
foreach ($directory_data as $row)
{
	...
	'S_DID'			=> $row['id'],
	'TITLE'			=> $row['title'],
	'U_TITLE'		=> append_sid("{$tonjaw_admin_path}whatsondetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}whatsondetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}whatson.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
