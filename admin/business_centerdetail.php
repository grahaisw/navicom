...
/**
*
* admin/business_centerdetail.php
*
...
$u_action = $tonjaw_admin_path . 'business_centerdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'business_centers_image'		=> $image,
	'business_centers_image_enabled'	=> (int) $image_enabled,
	'business_centers_clip'		=> $clip,
	'business_centers_clip_enabled'	=> (int) $clip_enabled,
	'business_centers_order'		=> (int) ($order)? $order : '1',
	'business_centers_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . BUSINESS_CENTERS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . BUSINESS_CENTERS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE business_centers_id = $did";
	...
	$business_centers_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$business_centers_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'business_centers_id'		=> (int) $did,
	    'translation_title'		=> (string) $business_centers_title,
	    'translation_description'	=> (string) $business_centers_content,
	...
	{
	    $sql = 'INSERT INTO ' . BUSINESS_CENTER_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . BUSINESS_CENTER_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'business_center.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($did))
    {
	die('Missing Fitness ID. Cannot update Fitnesses Table.');
    }
    // Grab fitness data
    $sql = 'SELECT * FROM ' . BUSINESS_CENTERS_TABLE . " WHERE business_centers_id = $did" ;
    ...
    $data_thumbnail = ($data['business_centers_image'])? $data['business_centers_image'] : '0';
    $data_clip = ($data['business_centers_clip'])? $data['business_centers_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['business_centers_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . BUSINESS_CENTER_TRANSLATIONS_TABLE . " WHERE business_centers_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}business_centerdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['business_centers_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['business_centers_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['business_centers_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['business_centers_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['business_centers_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['business_centers_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['business_centers_order'],
	    'S_CLIP_ENABLED'	=> ($data['business_centers_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['business_centers_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['business_centers_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}business_centerdetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_business_centerform.tpl',
));
