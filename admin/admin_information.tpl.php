    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<?php if ($this->_rootref['S_INFO']) {  $_info_count = (isset($this->_tpldata['info'])) ? sizeof($this->_tpldata['info']) : 0;if ($_info_count) {for ($_info_i = 0; $_info_i < $_info_count; ++$_info_i){$_info_val = &$this->_tpldata['info'][$_info_i]; ?>
			  ...
			  <tr>
			    <td><a href="<?php echo $_valas_val['U_NAME']; ?>"><?php echo $_info_val['NAME']; ?></a></td>
			    <td><?php echo $_info_val['NOMOR']; ?> <input type="hidden" name="info_id[]" value="<?php echo $_info_val['V_TID']; ?>"/></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_info_val['U_UPDATE']; ?>"><img src="<?php echo $_info_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_info_val['L_UPDATE']; ?>" title="<?php echo $_info_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			   
			    <td style="width: 5%" align="center"><a href="<?php echo $_info_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_info_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_info_val['L_DELETE']; ?>" title="<?php echo $_info_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
