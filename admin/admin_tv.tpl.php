...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<thead>
			<tr>
			  <th width="8%"><?php echo ((isset($this->_rootref['L_ORDER'])) ? $this->_rootref['L_ORDER'] : ((isset($user->lang['ORDER'])) ? $user->lang['ORDER'] : '{ ORDER }')); ?></th>
			...
			<tbody>
			<?php if ($this->_rootref['S_TVS']) {  $_tv_count = (isset($this->_tpldata['tv'])) ? sizeof($this->_tpldata['tv']) : 0;if ($_tv_count) {for ($_tv_i = 0; $_tv_i < $_tv_count; ++$_tv_i){$_tv_val = &$this->_tpldata['tv'][$_tv_i]; if (!($_tv_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="text" name="order_<?php echo $_tv_val['S_TID']; ?>" value="<?php echo $_tv_val['ORDER']; ?>"  maxlength="3" size="5" /></td>
			    <?php } else { ?>

			    <td><?php echo $_tv_val['ORDER']; ?></td>
			    <?php } ?>

			    <td><a href="<?php echo $_tv_val['U_NAME']; ?>"><img src="<?php echo $_tv_val['THUMBNAIL']; ?>" alt="<?php echo $_tv_val['NAME']; ?>" height="60"></a></td>
			    <td><strong><a href="<?php echo $_tv_val['U_NAME']; ?>"><?php echo $_tv_val['NAME']; ?></a></strong><br/><?php echo $_tv_val['URL']; ?></td>
			    <td><?php echo $_tv_val['GROUP']; ?><input type="hidden" name="tv_id[]" value="<?php echo $_tv_val['S_TID']; ?>"/></td>
			    <td><?php echo $_tv_val['ALLOW_ADS']; ?></td>
