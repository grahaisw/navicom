...
$u_action = $tonjaw_admin_path . 'fe_menudetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'menu_url'			=> $url,
	'menu_order'			=> (int) ($order)? $order : '0',
	'menu_in_mobile'		=> (int) $in_mobile_flag,
	'menu_in_stb'			=> (int) $in_stb_flag,
	'menu_in_empty_room'		=> (int) $in_empty_room_flag,
	'menu_enabled'			=> (int) $enabled_flag,
	'menu_thumbnail'		=> $thumbnail,
	'menu_runningtext_enabled'	=> (int) $runningtext_enabled_flag,
	'menu_group_id'			=> (int) $gid,
    );
...
*/
	...
	$sql = 'INSERT INTO ' . MENUS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	...
	{
	    $sql = 'INSERT INTO ' . MENU_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . MENU_TRANSLATIONS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'fe_menu.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    if (empty($mid))
    {
	die('Missing Menu ID. Cannot update Menus Table.');
    }

    $label = ($mode === 'update') ? $adm_lang['update_item'] : $adm_lang['view_item'];
    
    // Grab directory data
    $sql = 'SELECT * FROM ' . MENUS_TABLE . " WHERE menu_id = $mid" ;
    ...
    {
	//Get Menu Group
	$sql = "SELECT t.translation_title AS group_name FROM " . MENU_GROUPS_TABLE . " m, " . 
	    MENU_GROUP_TRANSLATIONS_TABLE . " t WHERE m.menu_group_id=t.menu_group_id AND m.menu_group_id=" . $data['menu_group_id'] . " AND t.language_id='" . $config['default_language'] . "'";
...
*/    
    $sql = 'SELECT * FROM ' . MENU_TRANSLATIONS_TABLE . " WHERE menu_id = $mid";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['menu_thumbnail'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'S_ORDER'			=> $data['menu_order'],
	    'S_URL'			=> $data['menu_url'],
	    'S_GROUP'			=> generate_menu_groups('group_id', $data['menu_group_id']),
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_IN_STB'		=> ($data['menu_in_stb'])? 'checked' : '',
	    'V_IN_MOBILE'		=> ($data['menu_in_mobile'])? 'checked' : '',
	    'V_IN_EMPTY_ROOM'		=> ($data['menu_in_empty_room'])? 'checked' : '',
	    'V_ENABLED'			=> ($data['menu_enabled'])? 'checked' : '',
	    'V_RUNNINGTEXT_ENABLED'	=> ($data['menu_runningtext_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_ORDER'		=> $data['menu_order'],
	    'S_GROUP'		=> $group_name,
	    'S_IN_STB'		=> ($data['menu_in_stb'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IN_MOBILE'	=> ($data['menu_in_mobile'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IN_EMPTY_ROOM'	=> ($data['menu_in_empty_room'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['menu_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}fe_menudetail.$phpEx", "mode=update") . '&amp;id=' .$mid . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
