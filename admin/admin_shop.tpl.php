...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tbody>
			<?php if ($this->_rootref['S_SHOPS']) {  $_shop_count = (isset($this->_tpldata['shop'])) ? sizeof($this->_tpldata['shop']) : 0;if ($_shop_count) {for ($_shop_i = 0; $_shop_i < $_shop_count; ++$_shop_i){$_shop_val = &$this->_tpldata['shop'][$_shop_i]; if (!($_shop_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_shop_val['ORDER']; ?></td>
			    <td><a href="<?php echo $_shop_val['U_NAME']; ?>"><img src="<?php echo $_shop_val['THUMBNAIL']; ?>" alt="<?php echo $_shop_val['NAME']; ?>" width="60"></a></td>
			    <td><strong><a href="<?php echo $_shop_val['U_NAME']; ?>"><?php echo $_shop_val['NAME']; ?></a></strong>
				<br/><?php echo $_shop_val['DESCRIPTION']; ?></td>
			    <td><?php echo $_shop_val['GROUP']; ?><input type="hidden" name="shop_id[]" value="<?php echo $_shop_val['S_RID']; ?>"/></td>
			    <td><?php echo $_shop_val['PRICE']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_shop_val['S_RID']; ?>" <?php echo $_shop_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_shop_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_shop_val['U_UPDATE']; ?>"><img src="<?php echo $_shop_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_shop_val['L_UPDATE']; ?>" title="<?php echo $_shop_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_shop_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_shop_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_shop_val['L_DELETE']; ?>" title="<?php echo $_shop_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
