    ...
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<tr>
			  ...
			  <th><?php echo ((isset($this->_rootref['L_URL'])) ? $this->_rootref['L_URL'] : ((isset($user->lang['URL'])) ? $user->lang['URL'] : '{ URL }')); ?></th>
			  <th><?php echo ((isset($this->_rootref['L_IN_MOBILE'])) ? $this->_rootref['L_IN_MOBILE'] : ((isset($user->lang['IN_MOBILE'])) ? $user->lang['IN_MOBILE'] : '{ IN_MOBILE }')); ?></th>
			...
			<tbody>
			<?php if ($this->_rootref['S_MENU']) {  $_menu_count = (isset($this->_tpldata['menu'])) ? sizeof($this->_tpldata['menu']) : 0;if ($_menu_count) {for ($_menu_i = 0; $_menu_i < $_menu_count; ++$_menu_i){$_menu_val = &$this->_tpldata['menu'][$_menu_i]; if (!($_menu_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>

			    <td><?php echo $_menu_val['ORDER']; ?></td>
			    <td><a href="<?php echo $_menu_val['U_TITLE']; ?>"><?php echo $_menu_val['TITLE']; ?></a>
			    <input type="hidden" name="menu_id[]" value="<?php echo $_menu_val['S_NID']; ?>"/></td>
			    <td><?php echo $_menu_val['THUMBNAIL']; ?></td>
			    <td><?php echo $_menu_val['URL']; ?></td>
			    <td><?php echo $_menu_val['IN_MOBILE']; ?></td>
			    <?php if ($this->_rootref['S_ADD_UPDATE']) {  ?><td style="width: 5%" align="center">
			    <input type="checkbox" name="mark_<?php echo $_menu_val['S_NID']; ?>" <?php echo $_menu_val['V_ENABLED']; ?>/><label>&nbsp;</label></td>
			    <?php } else { ?>

			    <td><?php echo $_menu_val['ENABLED']; ?></td>
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_menu_val['U_UPDATE']; ?>"><img src="<?php echo $_menu_val['ICON_PATH']; ?>/edit.png" alt="<?php echo $_menu_val['L_UPDATE']; ?>" title="<?php echo $_menu_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>

			    <td style="width: 5%" align="center"><a href="<?php echo $_menu_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_menu_val['ICON_PATH']; ?>/delete.png" alt="<?php echo $_menu_val['L_DELETE']; ?>" title="<?php echo $_menu_val['L_DELETE']; ?>" /></a></td>
			    <?php } ?>
