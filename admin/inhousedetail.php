...
$u_action = $tonjaw_admin_path . 'inhousedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'inhouses_image'		=> $image,
	'inhouses_image_enabled'	=> (int) $image_enabled,
	'inhouses_clip'		=> $clip,
	'inhouses_clip_enabled'	=> (int) $clip_enabled,
	'inhouses_order'		=> (int) ($order)? $order : '1',
	'inhouses_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . INHOUSES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . INHOUSES_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE inhouses_id = $did";
	...
	$inhouses_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$inhouses_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'inhouses_id'		=> (int) $did,
	    'translation_title'		=> (string) $inhouses_title,
	    'translation_description'	=> (string) $inhouses_content,
	...
	{
	    $sql = 'INSERT INTO ' . INHOUSE_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . INHOUSE_TRANSLATIONS_TABLE . " SET " . 
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . INHOUSES_TABLE . " WHERE inhouses_id = $did" ;
    ...
    $data_thumbnail = ($data['inhouses_image'])? $data['inhouses_image'] : '0';
    $data_clip = ($data['inhouses_clip'])? $data['inhouses_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['inhouses_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . INHOUSE_TRANSLATIONS_TABLE . " WHERE inhouses_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['inhouses_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['inhouses_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['inhouses_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['inhouses_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['inhouses_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['inhouses_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['inhouses_order'],
	    'S_CLIP_ENABLED'	=> ($data['inhouses_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['inhouses_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['inhouses_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}inhousedetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
