...
$u_action = $tonjaw_admin_path . 'wakeup_calldetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	'wakeup_calls_image'		=> $image,
	'wakeup_calls_image_enabled'	=> (int) $image_enabled,
	'wakeup_calls_clip'		=> $clip,
	'wakeup_calls_clip_enabled'	=> (int) $clip_enabled,
	'wakeup_calls_order'		=> (int) ($order)? $order : '1',
	'wakeup_calls_enabled'		=> (int) $enabled_flag,
    );
	...
	$sql = 'INSERT INTO ' . WAKEUP_CALLS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . WAKEUP_CALLS_TABLE . " SET " . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE wakeup_calls_id = $did";
	...
	$wakeup_calls_title = utf8_normalize_nfc(request_var('title_' . $row['id'], '', true));
	$wakeup_calls_content = utf8_normalize_nfc(request_var('content_' . $row['id'], '', true));
	
	$sql_translation = array(
	    'wakeup_calls_id'		=> (int) $did,
	    'translation_title'		=> (string) $wakeup_calls_title,
	    'translation_description'	=> (string) $wakeup_calls_content,
	...
	{
	    $sql = 'INSERT INTO ' . WAKEUP_CALL_TRANSLATIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_translation);
	...
	if ( !empty($translation_id) )
	{
	    $sql = 'UPDATE ' . WAKEUP_CALL_TRANSLATIONS_TABLE . " SET " . 
    ...
    //exit;
    redirect($config['admin_path'] . 'wakeup_call.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab fitness data
    $sql = 'SELECT * FROM ' . WAKEUP_CALLS_TABLE . " WHERE wakeup_calls_id = $did" ;
    ...
    $data_thumbnail = ($data['wakeup_calls_image'])? $data['wakeup_calls_image'] : '0';
    $data_clip = ($data['wakeup_calls_clip'])? $data['wakeup_calls_clip'] : '0';
    $thumbnail = $tonjaw_root_path . $config['media_path'] . $config['wakeup_calls_image_path'] . $data_thumbnail;
    ...
    $sql = 'SELECT * FROM ' . WAKEUP_CALL_TRANSLATIONS_TABLE . " WHERE wakeup_calls_id = $did";
    //echo $sql; exit;
    $result = $db->sql_query($sql);
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}wakeup_calldetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'S_THUMBNAIL'		=> $data['wakeup_calls_image'],
	    'CLIP_FILE'			=> file_exists($clip)? '1' : '0',
	    'S_CLIP_FILE'		=> $clip,
	    'S_CLIP'			=> $data['wakeup_calls_clip'],
	    'S_FORM'			=> '1',
	    'L_UPLOAD'			=> $adm_lang['upload'] . ' ' . $adm_lang['image'],
	    'V_CLIP_ENABLED'		=> ($data['wakeup_calls_clip_enabled'])? 'checked' : '',
	    'V_THUMBNAIL_ENABLED'	=> ($data['wakeup_calls_image_enabled'])? 'checked' : '',
	    'S_ORDER'			=> $data['wakeup_calls_order'],
	    'L_SUBMIT'			=> $adm_lang['submit'],
	    'V_ENABLED'			=> ($data['wakeup_calls_enabled'])? 'checked' : '',
	...
	$template->assign_vars(array(
	    ...
	    'S_DETAIL'		=> '1',
	    'S_ORDER'		=> $data['wakeup_calls_order'],
	    'S_CLIP_ENABLED'	=> ($data['wakeup_calls_clip_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_IMAGE_ENABLED'	=> ($data['wakeup_calls_image_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'S_ENABLED'		=> ($data['wakeup_calls_enabled'])? $adm_lang['yes'] : $adm_lang['no'],
	    'L_EDIT'		=> $adm_lang['update'],
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}wakeup_calldetail.$phpEx", "mode=update") . '&amp;id=' .$did . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_wakeup_callform.tpl',
));
