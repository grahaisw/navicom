...
$u_action = $tonjaw_admin_path . 'ads_homedetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $sql_ary = array(
	    'ads_home_name'		=> $name,
	    'ads_home_description'	=> $description,
        'ads_home_image'		=> $image,
	    'ads_home_enabled'		=> (int) (!empty($enabled_flag)? '1' : '0'),
    );
    
    if ($mode === 'add')
    {
	$sql = 'INSERT INTO ' . ADS_HOME_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql_check = "SELECT ads_home_image FROM " . ADS_HOME_TABLE . " WHERE ads_home_id = '".$rid."'";
	$result_check = $db->sql_query($sql_check);
	$home_image = $db->sql_fetchfield('ads_home_image');
	
	// Delete previous file
	$file = $tonjaw_root_path . $config['media_path'] . $config['ads_home_path'] . $home_image; 
	if(is_file($file))
	unlink($file); 	

	$sql = 'UPDATE ' . ADS_HOME_TABLE . ' SET ' . 
	    $db->sql_build_array('UPDATE', $sql_ary) .
	    " WHERE ads_home_id = $rid";
	$db->sql_query($sql);
	
    }
   
    redirect($config['admin_path'] . 'ads_home.' . $phpEx, $sid);
}
...
if ($mode === 'update')
{
    ...
    $sql = 'SELECT * FROM ' . ADS_HOME_TABLE . " WHERE ads_home_id=" . (int) $rid;
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}ads_homedetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '0',
    'L_ADS_NAME'		=> $adm_lang['name'],
    'L_ADS_DESCRIPTION'	=> $adm_lang['description'],
    'L_IMAGE'			=> $adm_lang['image'],
    //'L_DURATION'			=> $adm_lang['duration'],
    'L_ENABLED'			=> $adm_lang['enabled'],
    'S_ADS_NAME'			=> $data['ads_home_name'],
    'S_ADS_DESCRIPTION'		=> $data['ads_home_description'],
    'S_IMAGE'			=> $data['ads_home_image'],
    'S_ENABLED'			=> ($data['ads_home_enabled'])? 'checked' : '',
