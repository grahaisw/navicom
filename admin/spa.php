...
$u_action = append_sid("{$tonjaw_admin_path}spa.$phpEx", "mode=update");
...
$start = grab_spa($spa_data, $spa_count);
...
if ($mode === 'update')
{
    ...
    $rid	= (isset($_REQUEST['spa_id'])) ? request_var('spa_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . SPAS_TABLE . ' 
	  SET spa_enabled=' . (string) $mark[$i] ."
	  WHERE spa_id = '" . $rid[$i] . "'";
...
$thumbnail_path = $tonjaw_root_path . $config['media_path'] . $config['spa_icon_path'];
...
{
    //$data = array();
    $thumbnail = file_exists($thumbnail_path.$row['thumbnail'])? $thumbnail_path.$row['thumbnail'] : $thumbnail_path.$config['default_thumbnail_spa'];
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}spadetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_NAME'		=> append_sid("{$tonjaw_admin_path}spadetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}spa.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}spadetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '1',
    'S_THIRD_FIELD'		=> '1',
