...
$(function() {
    $('#DeleteLink').click(function() {
        return confirm('<?php echo ((isset($this->_rootref['L_CONFIRM_DELETE'])) ? $this->_rootref['L_CONFIRM_DELETE'] : ((isset($user->lang['CONFIRM_DELETE'])) ? $user->lang['CONFIRM_DELETE'] : '{ CONFIRM_DELETE }')); ?>');
			...
			<thead>
			<tr>
			  <th><?php echo ((isset($this->_rootref['L_COMPANY_NAME'])) ? $this->_rootref['L_COMPANY_NAME'] : ((isset($user->lang['COMPANY_NAME'])) ? $user->lang['COMPANY_NAME'] : '{ COMPANY_NAME }')); ?></th>
			  <th><?php echo ((isset($this->_rootref['L_CONTACT_PERSON'])) ? $this->_rootref['L_CONTACT_PERSON'] : ((isset($user->lang['CONTACT_PERSON'])) ? $user->lang['CONTACT_PERSON'] : '{ CONTACT_PERSON }')); ?></th>
			  <th><?php echo ((isset($this->_rootref['L_PHONE'])) ? $this->_rootref['L_PHONE'] : ((isset($user->lang['PHONE'])) ? $user->lang['PHONE'] : '{ PHONE }')); ?></th>
              <th><?php echo ((isset($this->_rootref['L_EMAIL'])) ? $this->_rootref['L_EMAIL'] : ((isset($user->lang['EMAIL'])) ? $user->lang['EMAIL'] : '{ EMAIL }')); ?></th>
			...
			<tbody>
			<?php if ($this->_rootref['S_ROOMS']) {  $_ads_count = (isset($this->_tpldata['ads'])) ? sizeof($this->_tpldata['ads']) : 0;if ($_ads_count) {for ($_ads_i = 0; $_ads_i < $_ads_count; ++$_ads_i){$_ads_val = &$this->_tpldata['ads'][$_ads_i]; if (!($_ads_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="bg1"><?php } else { ?><tr class="bg2"><?php } ?>
			    <td><?php echo $_ads_val['COMPANY_NAME']; ?></td>
			    <td><?php echo $_ads_val['CONTACT_PERSON']; ?></td>
			    <td><?php echo $_ads_val['PHONE']; ?></td>
			    <td><?php echo $_ads_val['EMAIL']; ?></td>
			    ...
			    <?php } if ($this->_rootref['S_ADD_UPDATE']) {  ?>
			    <td style="width: 5%" align="center"><a href="<?php echo $_ads_val['U_UPDATE']; ?>" rel="facebox"><img src="<?php echo $_ads_val['ICON_PATH']; ?>/icon_edit.gif" alt="<?php echo $_ads_val['L_UPDATE']; ?>" title="<?php echo $_ads_val['L_UPDATE']; ?>" /></a></td>
			    <?php } if ($this->_rootref['S_DELETE']) {  ?>
			    <td style="width: 5%" align="center"><a href="<?php echo $_ads_val['U_DELETE']; ?>" id="DeleteLink"><img src="<?php echo $_ads_val['ICON_PATH']; ?>/icon_delete.gif" alt="<?php echo $_ads_val['L_DELETE']; ?>" title="<?php echo $_ads_val['L_DELETE']; ?>" /></a>
			    </td>
			    <?php } ?>
			  </tr>
			<?php }} } ?>
