...
$u_action = append_sid("{$tonjaw_admin_path}recreational.$phpEx", "mode=update");
...
$start = grab_directory_grouping($directory_data, $directory_count, "recreational");
//print_r($start); exit;
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['recreationals_id'])) ? request_var('recreationals_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . RECREATIONALS_TABLE . ' 
	  SET recreationals_enabled=' . (string) $mark[$i] ."
	  WHERE recreationals_id = '" . $mid[$i] . "'";
...
foreach ($directory_data as $row)
{
	...
	'S_DID'			=> $row['id'],
	'TITLE'			=> $row['title'],
	'U_TITLE'		=> append_sid("{$tonjaw_admin_path}recreationaldetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}recreationaldetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}recreational.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
