...
$u_action = $tonjaw_admin_path . 'tvdetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    $url_udp = request_var('url_udp', '');
    $url_http = request_var('url_http', '');
    $gid	= array();
    ...
    $sql_ary = array(
	    ...
	    'tv_channel_url_udp'	=> $url_udp,
	    'tv_channel_url_http'	=> $url_http,
	    'tv_channel_thumbnail'	=> $thumbnail,
	    'tv_channel_order'		=> (int) ($order)? $order : '0',
	    //'tv_channel_enabled'	=> (int) ($enabled_flag)? '1' : '0',
	    //'tv_channel_allow_ads'	=> (int) ($allow_ads_flag)? '1' : '0',
	    'tv_channel_enabled'	=> (int) $enabled_flag,
	    'tv_channel_allow_ads'	=> (int) $allow_ads_flag,
    );

    if ($mode === 'add')
    {
	$sql = 'INSERT INTO ' . TV_CHANNELS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
    ...
    if ($mode === 'update')
    {
	$sql = 'UPDATE ' . TV_CHANNELS_TABLE . " SET " . 
    ...
    redirect($config['admin_path'] . 'tv.' . $phpEx, $sid);
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    // Grab tv data
    $sql = 'SELECT * FROM ' . TV_CHANNELS_TABLE . " WHERE tv_channel_id = $tid" ;
...
$template->assign_vars(array(
    ...
    'L_URL_UDP'			=> $adm_lang['url_udp'],
    'L_URL_HTTP'		=> $adm_lang['url_http'],
    'L_ADD'			=> $adm_lang['add'],
    ...
    'S_NAME'			=> $data['tv_channel_name'],
    'S_URL_UDP'			=> $data['tv_channel_url_udp'],
    'S_URL_HTTP'		=> $data['tv_channel_url_http'],
    'L_LABEL'			=> $label,
));
