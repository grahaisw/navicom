...
$u_action = $tonjaw_admin_path . 'popup_promodetail.' . $phpEx .'?sid=' . $sid;
...
if (isset($_POST['submit']))
{
    ...
    {
	$error = true;
	$error_msg = 'Pop Up Image field cannot be left empty.';
	...
	$sql_ary = array(
	    ...
	    'popup_enabled'		=> (int) $enabled_flag,
	    'popup_image'		=> (string) $image,
	    'popup_title'		=> (string) $title,
	    'popup_description'	=> (string) $description,
	);

	if ($mode === 'add')
	{
	  
	    $sql = 'INSERT INTO ' . POPUP_PROMOS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
	...
	if ($mode === 'update')
	{
	    $sql = 'UPDATE ' . POPUP_PROMOS_TABLE . " SET " . 
     ...
     redirect($config['admin_path'] . 'popup_promo.' . $phpEx, $sid);
}
...
if ($mode === 'update' || $mode === 'detail')
{
    ...
    $sql = 'SELECT * FROM ' . POPUP_PROMOS_TABLE . " WHERE popup_id=" . (int) $aid;
...
$template->assign_vars(array(
    ...
    'L_LABEL'			=> $label,
    'S_IMAGE'			=> $data['popup_image'],
    'S_TITLE'			=> prepare_message($data['popup_title']),
    'S_DESCRIPTION'		=> prepare_message($data['popup_description']),
));
...
switch( $mode )
{
	...
	$template->assign_vars(array(
	    ...
	    'U_EDIT'		=> append_sid("{$tonjaw_admin_path}popup_promodetail.$phpEx", "mode=update") . '&amp;id=' .$aid . '&amp;parent=' . $parent . '&amp;module=' . $module->active_module_name,
	));
...
$template->set_filenames(array(
	'body' => 'admin_popup_promoform.tpl',
));
