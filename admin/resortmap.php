...
$u_action = append_sid("{$tonjaw_admin_path}resortmap.$phpEx", "mode=update");
...
$start = grab_directory_grouping($directory_data, $directory_count, "resortmap");
//print_r($start); exit;
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['resortmaps_id'])) ? request_var('resortmaps_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . RESORTMAPS_TABLE . ' 
	  SET resortmaps_enabled=' . (string) $mark[$i] ."
	  WHERE resortmaps_id = '" . $mid[$i] . "'";
...
if (isset($_GET['id']) && $mode === 'delete')
{
    ...
    $sql = 'DELETE FROM ' . RESORTMAPS_TABLE . ' WHERE resortmaps_id = ' . (int) $nid;
    $db->sql_query($sql);
...
foreach ($directory_data as $row)
{
	...
	'S_DID'			=> $row['id'],
	'TITLE'			=> $row['title'],
	'U_TITLE'		=> append_sid("{$tonjaw_admin_path}resortmapdetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}resortmapdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}resortmap.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
