...
$u_action = append_sid("{$tonjaw_admin_path}tour.$phpEx", "mode=update");
...
$start = grab_tour($tour_data, $tour_count);
...
if ($mode === 'update')
{
    ...
    $rid	= (isset($_REQUEST['tour_id'])) ? request_var('tour_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . TOURS_TABLE . ' 
	  SET tour_enabled=' . (string) $mark[$i] ."
	  WHERE tour_id = '" . $rid[$i] . "'";
...
$thumbnail_path = $tonjaw_root_path . $config['media_path'] . $config['tour_icon_path'];
...
foreach ($tour_data as $row)
{
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}tourdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_NAME'		=> append_sid("{$tonjaw_admin_path}tourdetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}tour.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
...
$template->assign_vars(array(
    ...
    'U_ADD'			=> append_sid("{$tonjaw_admin_path}tourdetail.$phpEx", "mode=add") . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
    'L_ADD'			=> $adm_lang['add'],
    'S_FACEBOX'			=> '0',
    'S_DATATABLE_NODES'		=> '1',
    'S_THIRD_FIELD'		=> '1',
