...
$u_action = append_sid("{$tonjaw_admin_path}drop_pickup.$phpEx", "mode=update");
...
$start = grab_directory_grouping($directory_data, $directory_count, "drop_pickup");
//print_r($start); exit;
...
if ($mode === 'update')
{
    ...
    $mid	= (isset($_REQUEST['drop_pickups_id'])) ? request_var('drop_pickups_id', array('0')) : array();
	...
	$sql = 'UPDATE ' . DROP_PICKUPS_TABLE . ' 
	  SET drop_pickups_enabled=' . (string) $mark[$i] ."
	  WHERE drop_pickups_id = '" . $mid[$i] . "'";
...
foreach ($directory_data as $row)
{
	...
	'S_DID'			=> $row['id'],
	'TITLE'			=> $row['title'],
	'U_TITLE'		=> append_sid("{$tonjaw_admin_path}drop_pickupdetail.$phpEx", "mode=detail") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	...
	'U_UPDATE'		=> append_sid("{$tonjaw_admin_path}drop_pickupdetail.$phpEx", "mode=update") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
	'L_UPDATE'		=> $adm_lang['edit'],
	'U_DELETE'		=> append_sid("{$tonjaw_admin_path}drop_pickup.$phpEx", "mode=delete") . '&amp;id=' . $row['id'] . '&amp;parent=' . $file[0] . '&amp;module=' . $module->active_module_name,
