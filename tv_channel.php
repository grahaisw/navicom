...
//Get guest last channel
$last_channel_sql = "SELECT node_last_channel FROM " . NODES_TABLE . " WHERE node_ip = '" . $session->ip."'";
$last_channel_result = $db->sql_query($last_channel_sql);
$guest_last_channel = $db->sql_fetchfield('node_last_channel');

if(empty($guest_last_channel)) {
    $guest_last_channel = $config['tv_channel_id_on_home'];
}
...
if ( !$config['tv_grouping'] || !empty($gid) )
{
    ...
    */
	...
	if( $row['tv_channel_id'] != $temp_id)
	{
	    ...
	    if($row['tv_channel_id'] == $guest_last_channel) {
                $guest_last_channel_index = $i;
            }
    ...
    */
    ...
    $db->sql_freeresult($result);
	if ($gid == 6){
           $bground_ch = 'bground-channel-all.jpg';
        } else {
           $bground_ch = 'bground-channel.jpg';
        }
	$j = 0;
  ...
  'S_WIDGET_DATE'		=> date("D. j M"),
	...
	'S_GUEST_LAST_CHANNEL'	=> $guest_last_channel_index,
  ));
...
if ( empty($gid) && $mode !== 'ch' && $config['tv_grouping'] )
{
	...
	$sql1 = 'SELECT COUNT(t.tv_channel_id) AS total_entries
		...
		WHERE  g.tv_channel_group_id=' . $row['tv_channel_group_id'].' AND t.tv_channel_enabled = 1';
	//echo $sql1; exit;
	...
	$sql = 'SELECT t.tv_channel_id, t.tv_channel_url_http, t.tv_channel_url_udp FROM ' . TV_CHANNELS_TABLE . ' t 
	    ...
	    WHERE g.tv_channel_group_id=' . $row['tv_channel_group_id'] .' AND t.tv_channel_enabled = 1 ORDER BY t.tv_channel_order';
	//echo $sql; exit;
	...
	if($guestgroup == 'GM & HM meeting') {
	$groupmovie = '["Group Movie", "225.1.1.5","./media/images/tv/group/gm.png", 0, "./gm.php"],';
	}
	//print_r($source_url);
	...
	//{
		...
		'http'		=> $source_url[0]['http'],
		'udp'		=> $source_url[0]['udp'],
	    );
	//}
